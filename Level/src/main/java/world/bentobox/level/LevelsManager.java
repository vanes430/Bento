...
public class LevelsManager {
	...
	// Cache for top tens
	private Map<World, CachedData> cache = new ConcurrentHashMap<>();
	 ...
	 * 
	 * @param targetPlayer
	 *            - uuid of targeted player - owner or team member
	 * @param island
	 *            - island to calculate
	...
	public long getInitialCount(Island island) {
		Long initialLevel = getLevelsData(island).getInitialLevel(); // Backward compatibility check. For all new
																		// islands, this should be null.
		Long initialCount = getLevelsData(island).getInitialCount();
		if (initialLevel != null) {
			// Initial level exists so convert it
			if (initialCount == null) { // If initialCount is not null, then this is an edge case and initialCount will
										// be used and initialLevel discarded
	...
	/**
	 * Runs the level calculation using the current formula until the level matches
	 * the initial value, or fails.
	 * 
	 * @param initialLevel
	 *            - the old initial level
	 * @return block count to obtain this level now
	 * @throws ParseException
	 *             if the formula for level calc is bugged
	 * @throws IOException
	 *             if the number of blocks cannot be found for this level
	 */
	 ...
	 *            - target player UUID
	 * @param ownerOnly
	 *            - return level only if the target player is the owner
	 * @return Level of the player's island or zero if player is unknown or UUID is
	 *         null
	 */
	 ...
	 */
	@NonNull public IslandLevels getLevelsData(@NonNull Island island) {
	 ...
	 */
	@NonNull public Map<Island, Long> getWeightedTopTen(@NonNull World world, int size) {
	 ...
	 * @return sorted top ten map. The key is the island unique ID
	 */
	@NonNull public Map<String, Long> getTopTen(@NonNull World world, int size) {
		createAndCleanRankings(world);
	...
	public void loadTopTens() {
		...
		Bukkit.getAsyncScheduler().runNow(addon.getPlugin(), t -> {
