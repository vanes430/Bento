...
public class ConversationUtils {
	 ...
	 *
	 * @param consumer
	 *            Consumer that accepts player output text.
	 * @param question
	 *            Message that will be displayed in chat when player triggers
	 *            conversion.
	 * @param user
	 *            User who is targeted with current confirmation.
	 */
	public static void createStringInput(Consumer<String> consumer, User user, @NonNull String question,
		...
		new ConversationFactory(BentoBox.getInstance())
				.withPrefix(context -> user.getTranslation("level.conversations.prefix")).withFirstPrompt(stringPrompt).
	 ...
	 *
	 * @param consumer
	 *            Consumer which must return null value.
	 * @param user
	 *            User who was using conversation.
		...
		return abandonedEvent -> {
			if (!abandonedEvent.gracefulExit()) {
				...
				abandonedEvent.getContext().getForWhom()
						.sendRawMessage(user.getTranslation("level.conversations.prefix")
