...
public class Utils {
	...
	private Utils() {
	} // Private constructor as this is a utility class only with static methods

	/**
	 * This method sends a message to the user with appended "prefix" text before
	 * message.
	 * 
	 * @param user
	 *            User who receives message.
	 * @param translationText
	 *            Translation text of the message.
	 * @param parameters
	 *            Parameters for the translation text.
	 */
	public static void sendMessage(User user, String translationText, String... parameters) {
		user.sendMessage(
				user.getTranslation("level.conversations.prefix") + user.getTranslation(translationText, parameters));
	}

	/**
	 * This method gets string value of given permission prefix. If user does not
	 * have given permission or it have all (*), then return default value.
	 *
	 * @param user
	 *            User who's permission should be checked.
	 * @param permissionPrefix
	 *            Prefix that need to be found.
	 * @param defaultValue
	 *            Default value that will be returned if permission not found.
	...
	public static String getPermissionValue(User user, String permissionPrefix, String defaultValue) {
		if (user.isPlayer()) {
			...
			List<String> permissions = user.getEffectivePermissions().stream()
					...
					.filter(permission -> permission.startsWith(permPrefix)).toList();
	 ...
	 *
	 * @param values
	 *            Array that should be searched for given value.
	 * @param currentValue
	 *            Value which next element should be found.
	 * @param <T>
	 ...
	 *            Array that should be searched for given value.
	 * @param currentValue
	 *            Value which previous element should be found.
	 * @param <T>
	 *            Instance of given object.
