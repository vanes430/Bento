...
public class BlockConfig {
	 ...
	 * @param addon
	 *            - addon
	 * @param blockValuesConfig
	 *            - yaml configuration file containing the block values
	 * @param file
	 *            - the file representing the yaml config. Will be saved after
	 *            readong.
	 * @throws IOException
	...
	private void addMissing(ConfigurationSection blocks) {
		...
		Registry.MATERIAL.stream().filter(m -> m.isBlock()).filter(m -> !m.isAir())
		...
		Registry.MATERIAL.stream().filter(Material::isItem).filter(m -> m.name().endsWith("_SPAWN_EGG")) // Get
																											// potential
																											// spawners
																											// by
																											// looking
																											...
																											// can be
																											// set
				.map(m -> m.getKey().getKey().substring(0, m.name().length() - 10) + SPAWNER) // Change the name of the
																								// egg to
																								// "entity-type_spawner"
	 ...
	 * 
	 * @param obj
	 *            material, entity type, or namespacedId
	...
	/**
	 * Retrieves the value associated with a block in the specified world, using
	 * world-specific settings if available, or falling back to baseline values.
	 *
	 * @param world
	 *            the world context
	 * @param obj
	 *            the object representing the entity type or material
	 ...
	 * 
	 * @param obj
	 *            object that can be a material or string
