...
public class DetailsPanel {
	...
	private void updateFilters() {
		...
		if (this.activeTab == Tab.SPAWNER) {
			...
			blockCountList.addAll(materialCountMap.entrySet().stream()
					.map(entry -> new BlockRec(entry.getKey(), entry.getValue(), 0)).collect(Collectors.toList()));
		...
		switch (this.activeFilter) {
			...
			case VALUE -> {
				sorter = (o1, o2) -> {
					...
					long o1Value = (long) o1Count * this.addon.getBlockConfig().getBlockValues()
							.getOrDefault(o1.key().toString().toLowerCase(Locale.ENGLISH), 0);
					long o2Value = (long) o2Count * this.addon.getBlockConfig().getBlockValues()
							.getOrDefault(o2.key().toString().toLowerCase(Locale.ENGLISH), 0);
