...
public class TopLevelPanel {
	 ...
	 * 
	 * @param template
	 *            Template record for fallback button.
	 * @param index
	...
	private PanelItem createIslandIcon(ItemTemplateRecord template, IslandTopRecord islandTopRecord, int index) {
		...
		activeActions.removeIf(action -> {
			switch (action.actionType().toUpperCase()) {
				...
				case "VISIT" -> {
					return island.getOwner() == null || this.addon.getVisitHook() == null || !this.addon.getVisitHook()
							.getAddonManager().preprocessTeleportation(this.user, island, true);
				}
				case "VIEW" -> {
						...
						case "VISIT" ->
							...
							this.addon.getPlugin().getIWM().getAddon(this.world)
									.flatMap(GameModeAddon::getPlayerCommand).ifPresent(command -> {
										String mainCommand = this.addon.getVisitHook().getSettings()
												.getPlayerMainCommand();

										if (!mainCommand.isBlank()) {
											this.user.closeInventory();
											this.user.performCommand(command.getTopLabel() + " " + mainCommand + " "
													+ island.getOwner());
										}
									});
