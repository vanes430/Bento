...
public class ValuePanel {
	 ...
	 *            Level object
	 * @param world
	 *            World where user is operating
	 * @param user
	 *            User who opens panel
	 */
	private ValuePanel(Level addon, World world, User user) {
		this.addon = addon;
		this.world = world;
		this.user = user;
	...
	private PanelItem createSearchButton(ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
		...
		if (template.description() != null) {
			// Set description
			builder.description(
					this.user.getTranslation(this.world, template.description(), "[text]", this.searchText));
		}
		...
		activeActions.removeIf(action -> "CLEAR".equalsIgnoreCase(action.actionType()) && this.searchText.isBlank());
	 ...
	 *            The button template.
	 * @param blockCount
	 *            The block record containing count details.
	...
	private PanelItem createMaterialButton(ItemTemplateRecord template, BlockRecord blockCount) {
		...
		String underWater = underwaterMultiplier != 1.0
				? this.user.getTranslationOrNothing(baseKey + "underwater", TextVariables.NUMBER,
						String.valueOf(blockValue * underwaterMultiplier))
				: "";
