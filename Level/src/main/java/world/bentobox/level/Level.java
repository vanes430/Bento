...
public class Level extends Addon {
	...
	/**
	 * A helper that only executes the provided hookAction if the plugin is not
	 * disabled.
	 */
	...
	/**
	 * Sets the player's level to a value. This will only last until the player
	 * reruns the level command
	 *
	 * @param world
	 *            - world
	 * @param targetPlayer
	 ...
	 *            - island
	 * @param level
	 *            - initial calculated island level
	 */
	/*
	 * TODO public void setInitialIslandLevel(@NonNull Island island, long level) {
	 ...
	 */
	/*
	 * TODO public long getInitialIslandLevel(@NonNull Island island) { return
	 * getManager().getInitialLevel(island); }
	 */
	 ...
	 * 
	 * @param world
	 *            - the world where this island is
	 * @param user
	 *            - not used! See depecration message
	 * @param playerUUID
	 *            - the target island member's UUID
	 ...
	 */
	/*
	 * @Deprecated(since = "2.3.0", forRemoval = true) public LevelsData
	 * getLevelsData(UUID targetPlayer) { LevelsData ld = new
	 * LevelsData(targetPlayer);
	 * getPlugin().getAddonsManager().getGameModeAddons().stream() .filter(gm ->
	 * !settings.getGameModes().contains(gm.getDescription().getName())).forEach(gm
	 * -> { if (getSettings().isZeroNewIslandLevels()) { Island island =
	 * getIslands().getIsland(gm.getOverWorld(), targetPlayer); if (island != null)
	 * { ld.setInitialLevel(gm.getOverWorld(), this.getInitialIslandLevel(island));
	 * } } ld.setLevel(gm.getOverWorld(), this.getIslandLevel(gm.getOverWorld(),
	 * targetPlayer)); }); return ld; }
	 */
