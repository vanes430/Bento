...
public class Pipeliner {
	...
	public Pipeliner(Level addon) {
		...
		task = Bukkit.getGlobalRegionScheduler().runAtFixedRate(BentoBox.getInstance(), t -> {
			...
			if (!inProcessQueue.isEmpty() || toProcessQueue.isEmpty())
				return;
	...
	/**
	 * Adds an island to the scanning queue but only if the island is not already in
	 * the queue
	 * 
	 * @param island
	 *            - the island to scan
	 * @return CompletableFuture of the results. Results will be null if the island
	 *         is already in the queue
	 */
	public CompletableFuture<Results> addIsland(Island island) {
		// Check if queue already contains island and it's not an island zero
		// calculation
	 ...
	 * @return the task
	 */
	protected ScheduledTask getTask() {
		return task;
	}
