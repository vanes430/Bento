...
/**
 * This listener checks when BentoBox is ready and then tries to migrate Levels
 * addon database, if it is required.
 */
public class MigrationListener implements Listener {
	...
	public void onBentoBoxReady(BentoBoxReadyEvent e) {
		 ...
		 * NewIsland.builder().addon(gm).player(User.getInstance(UUID.randomUUID())).
		 * name("default").reason(Reason.CREATE).noPaste().build(); } catch (IOException
