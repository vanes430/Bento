...
/**
 * Stores the levels data of the island. A note - if this class is extended to
 * support new exposed fields and legacy data doesn't include those fields they
 * will be set to null by GSON. They will not be initialized and if any attempt
 * is made to use them, then the JVM will give up WITHOUT AN ERROR!!! That is
 * why there are null checks throughout this class.
 *
 * @author tastybento
 *
 */
...
public class IslandLevels implements DataObject {
	 ...
	 */
	/*
	 * public long getInitialLevel() { // TODO: Add Backwards compatibility return
	 * initialLevel; }
	 * 
	 * /**
	 * 
	 ...
	 */
	/*
	 * public void setInitialLevel(long initialLevel) { this.initialLevel =
	 * initialLevel; }
	 */
	/**
	...
	public Map<Object, Integer> getUwCount() {
		// Loaded objects come in as strings, so need to be converted to Material Or
		// EntityTypes
		...
		for (Map.Entry<Object, Integer> entry : blockCountMap.entrySet()) {
			...
			if (key instanceof String keyStr) {
				...
				if (material != null) {
					...
					// Fallback to converting to EntityType (using uppercase as enum constants are
					// uppercase)
					try {
	...
	/**
	 * @return the initialLevel
	 * @deprecated only used for backwards compatibility. Use
	 *             {@link #getInitialCount()} instead
	 * @since 1.0
	 */
