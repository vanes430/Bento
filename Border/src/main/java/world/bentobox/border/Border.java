...
/**
 * Border add-on entry point that wires commands, listeners, and per-player
 * border rendering for supported game modes.
 * <p>
 * Lifecycle:
 * <ul>
 * <li>{@link #onLoad()} loads and persists configuration.</li>
 * <li>{@link #onEnable()} hooks into available game modes, registers commands
 * and listeners, and builds the border display implementation.</li>
 * </ul>
 */
public class Border extends Addon {
	...
	private void registerPlaceholders() {
		...
		this.getPlugin().getPlaceholdersManager().registerPlaceholder(this, "type",
				user -> BorderType
						.fromId(user.getMetaData(PerPlayerBorderProxy.BORDER_BORDERTYPE_META_DATA)
								.orElse(new MetaDataValue(getSettings().getType().getId())).asByte())
						.orElse(getSettings().getType()).getCommandLabel());
		this.getPlugin().getPlaceholdersManager().registerPlaceholder(this, "color", user -> {
			if (user == null) {
				return "";
			}
