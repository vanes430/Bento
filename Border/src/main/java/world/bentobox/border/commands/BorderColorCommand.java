...
/**
 * Command to allow players with the appropriate permission to set their own
 * border color. Permission required: [gamemode].border.color.[color] e.g.
 * bskyblock.border.color.red
 */
public final class BorderColorCommand extends CompositeCommand {

	private static final List<String> COLOR_NAMES = Arrays.stream(BorderColor.values()).map(c -> c.name().toLowerCase())
			.toList();
	...
	public Optional<List<String>> tabComplete(User user, String alias, List<String> args) {
		String permPrefix = getPlugin().getIWM().getPermissionPrefix(getWorld());
		return Optional
				.of(COLOR_NAMES.stream().filter(c -> user.hasPermission(permPrefix + "border.color." + c)).toList());
	}
}
