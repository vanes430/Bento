...
public class IslandDataManager {
	...
	public IslandDataManager(StoneGeneratorAddon addon) {
		this.addon = addon;
	}
	public void addWorld(org.bukkit.World w) {
		if (w != null)
			worlds.add(w);
	}
	public boolean canOperate(org.bukkit.World w) {
	...
	public @Nullable GeneratorDataObject validate(@Nullable Island island) {
		...
		data.getActiveGeneratorList().removeIf(
				id -> addon.getAddonManager().getGeneratorByID(id) == null || !data.getUnlockedTiers().contains(id));
		return data;
	}
	...
	private void checkDefaultGenerator(Island island, GeneratorDataObject data) {
		addon.getAddonManager().getAllTiers(island.getWorld()).stream().filter(GeneratorTierObject::isDefaultGenerator)
				.findFirst().ifPresent(defaultTier -> {
