...
public class PanelComponents {
	public static void fillTiers(GeneratorUserPanel p, Map<Integer, PanelItem> items, User u) {
		for (int i = 0; i < 5; i++) {
			...
			PanelItemBuilder b = new PanelItemBuilder()
					.icon(owned ? t.getGeneratorIcon() : new ItemStack(Material.GRAY_DYE))
	...
	public static void fillActions(GeneratorUserPanel p, Map<Integer, PanelItem> items, User u) {
		...
		int cur = -1;
		for (int i = 0; i < p.tiers.size(); i++)
			if (p.data.getPurchasedTiers().contains(p.tiers.get(i).getUniqueId()))
				cur = i;
		final int high = cur;
		boolean canActivate = high >= 0 && p.data.getActiveGeneratorList().isEmpty();
		items.put(18,
				new PanelItemBuilder().icon(hasActive ? Material.RED_DYE : Material.GREEN_DYE)
						.name(u.getTranslation(
								hasActive ? "stone-generator.ui.status_active" : "stone-generator.ui.status_inactive"))
	...
	private static void fillUpgrade(GeneratorUserPanel p, Map<Integer, PanelItem> items, int cur, User u) {
		...
		if (nextIdx >= p.tiers.size())
			items.put(26, new PanelItemBuilder().icon(Material.OBSIDIAN)
					.name(u.getTranslation("stone-generator.ui.max_level")).build());
		else {
			...
			items.put(26, new PanelItemBuilder().icon(Material.LIME_CONCRETE)
					.name(Util.translateColorCodes(
							u.getTranslation("stone-generator.ui.upgrade_to") + upgradeTier.getFriendlyName()))
					.description(Arrays.asList(
							Util.translateColorCodes(u.getTranslation("stone-generator.ui.cost") + "$"
									+ upgradeTier.getGeneratorTierCost()),
					...
					.clickHandler((panel, user, c, i) -> {
						...
						if (!p.addon.getAddonManager().canPurchase(user, p.data, upgradeTier)) {
							...
							Utils.sendMessage(user,
									u.getTranslation("stone-generator.messages.not-enough-money", "[number]",
											String.format("%.2f", cost), "[balance]", String.format("%.2f", balance)));
							return true;
						}
	...
	private static List<String> getLore(User u, GeneratorTierObject t, boolean owned, boolean active) {
		...
		t.getDescription().forEach(s -> l.add(Util.translateColorCodes(s)));
		l.add("");
		l.add(u.getTranslation("stone-generator.ui.blocks_title"));
		double tot = t.getBlockChanceMap().lastKey(), last = 0;
		for (var e : t.getBlockChanceMap().entrySet()) {
			double ch = (e.getKey() - last) / tot;
			l.add(Util.translateColorCodes(
					" &8- " + Utils.prettifyObject(e.getValue()) + ": &7" + String.format("%.1f%%", ch * 100)));
			last = e.getKey();
		}
		l.add("");
		l.add(active
				? u.getTranslation("stone-generator.gui.descriptions.generator.status.active")
				: (owned
						? u.getTranslation("stone-generator.ui.owned")
						: u.getTranslation("stone-generator.ui.locked") + "$" + t.getGeneratorTierCost()));
		return l;
	}

	private static String getLevelLabel(User u, int i) {
		return u.getTranslation("stone-generator.ui.level", "Level") + " " + (i + 1);
	}
}
