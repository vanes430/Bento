...
public class AcidBiomeProvider extends BiomeProvider {
	...
	public Biome getBiome(WorldInfo worldInfo, int x, int y, int z) {
		return switch (worldInfo.getEnvironment()) {
			...
			case THE_END -> {
				// Use THE_VOID biome if endGenerateStructures is false (void End)
				if (addon.getSettings().isEndGenerateStructures() == false) {
					yield Biome.THE_VOID;
				}
				yield addon.getSettings().getDefaultEndBiome();
			}
