...
public class AcidIsland extends GameModeAddon {
	...
	public void onLoad() {
		...
		saveDefaultConfig();
		// Save panels
		saveResource("panels/island_creation_panel.yml", false);
	...
	@NonNull public AISettings getSettings() {
	...
	public void createWorlds() {
		...
		if (settings.isNetherGenerate()) {
			...
			netherWorld = settings.isNetherIslands()
					? getWorld(worldName, World.Environment.NETHER, chunkGenerator)
	 ...
	 * @param env
	 *            - the environment
	 * @param chunkGenerator2
	 *            - the chunk generator. If <tt>null</tt> then the generator will
	 *            not be specified
	...
	private World getWorld(String worldName2, Environment env, @Nullable ChunkGenerator chunkGenerator2) {
		...
		// Add generator settings to all worlds to ensure they are void worlds
		// This prevents vanilla structures (like pillars) from spawning in the End.
		wc.generatorSettings(
				"{\"layers\":[{\"block\":\"minecraft:air\",\"height\":1}],\"biome\":\"minecraft:the_void\"}");

		if (chunkGenerator2 != null) {
			wc.generator(chunkGenerator2).biomeProvider(this.biomeProvider);
		}
	...
	public ChunkGenerator getDefaultWorldGenerator(String worldName, String id) {
		// This is a temporary diagnostic override.
		// Forcing to false for Overworld to check if unwanted
		// caves/decorations/structures are disabled.
		// Forcing to false for Nether and End if they are not island worlds.
		return new ChunkGeneratorWorld(this) {
			@Override
			public boolean shouldGenerateCaves() {
				// If it's the Overworld, force caves to false.
				// If it's Nether/End and they are not island worlds, force caves to false.
				if (World.Environment.NORMAL.equals(Bukkit.getWorld(worldName).getEnvironment())
						|| (World.Environment.NETHER.equals(Bukkit.getWorld(worldName).getEnvironment())
								&& !this.getAddon().getSettings().isNetherIslands())
						|| (World.Environment.THE_END.equals(Bukkit.getWorld(worldName).getEnvironment())
								&& !this.getAddon().getSettings().isEndIslands())) {
					return false;
				}
			...
			public boolean shouldGenerateDecorations() {
				// If it's the Overworld, force decorations to false.
				// If it's Nether/End and they are not island worlds, force decorations to
				// false.
				if (World.Environment.NORMAL.equals(Bukkit.getWorld(worldName).getEnvironment())
						|| (World.Environment.NETHER.equals(Bukkit.getWorld(worldName).getEnvironment())
								&& !this.getAddon().getSettings().isNetherIslands())
			...
			public boolean shouldGenerateStructures() {
				// If it's the Overworld, force structures to false.
				// If it's Nether/End and they are not island worlds, force structures to false.
				if (World.Environment.NORMAL.equals(Bukkit.getWorld(worldName).getEnvironment())
						|| (World.Environment.NETHER.equals(Bukkit.getWorld(worldName).getEnvironment())
								&& !this.getAddon().getSettings().isNetherIslands())
						|| (World.Environment.THE_END.equals(Bukkit.getWorld(worldName).getEnvironment())
