...
tasks.withType<JavaCompile> {
    ...
    // GraalVM Optimization Flags (Enterprise)
    options.isFork = true
...
dependencies {
    ...
    // CanvasMC API (for portal events) - compileOnly since it's provided by Canvas server
...
// Resolve capability conflict: CanvasMC API is preferred over Folia API
configurations.all {
    resolutionStrategy {
        dependencySubstitution {
            substitute(module("dev.folia:folia-api")).using(module("io.canvasmc.canvas:canvas-api:1.21.11-R0.1-SNAPSHOT"))
        }
    }
}
