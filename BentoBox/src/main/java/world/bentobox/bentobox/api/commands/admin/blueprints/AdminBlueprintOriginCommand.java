...
public class AdminBlueprintOriginCommand extends CompositeCommand {
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		if (b != null) {
			...
			if (user.isPlayer()) {
				user.getPlayer().getScheduler().runDelayed(getPlugin(),
						t -> user.getPlayer().sendBlockChange(b.getLocation(), b.getBlockData()), null, 20L);
			} else {
				Bukkit.getGlobalRegionScheduler().runDelayed(getPlugin(),
						t -> user.getPlayer().sendBlockChange(b.getLocation(), b.getBlockData()), 20L);
			}
