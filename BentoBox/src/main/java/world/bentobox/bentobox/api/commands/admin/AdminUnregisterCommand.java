...
public class AdminUnregisterCommand extends ConfirmableCommand {
	...
	void unregisterIsland(User user) {
		// Unregister island
		IslandEvent.builder().island(targetIsland).location(targetIsland.getCenter())
				.reason(IslandEvent.Reason.UNREGISTERED).involvedPlayer(targetUUID).admin(true).build();
		IslandEvent.builder().island(targetIsland).involvedPlayer(targetUUID).admin(true)
		...
		user.sendMessage("commands.admin.unregister.unregistered-island", TextVariables.XYZ,
				Util.xyz(targetIsland.getCenter().toVector()), TextVariables.NAME, getPlayers().getName(targetUUID));
	}
