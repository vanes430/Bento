...
public class AdminSettingsCommand extends CompositeCommand {
	 ...
	 *            - user
	 * @param string
	 *            - the rank given in the command line
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		// Player settings
		new TabbedPanelBuilder().user(user).world(island.getWorld()).island(island)
				.tab(1, new SettingsTab(getWorld(), user, Flag.Type.PROTECTION, Flag.Mode.EXPERT))
				.tab(2, new SettingsTab(getWorld(), user, Flag.Type.SETTING, Flag.Mode.EXPERT)).startingSlot(1).size(54)
				.build().openPanel();
		return true;
	}
