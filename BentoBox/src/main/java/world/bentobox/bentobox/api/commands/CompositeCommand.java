...
/**
 * The foundation for BentoBox's command system, implementing a composite
 * pattern for nested command hierarchies.
 * <p>
 * Key features:
 * <ul>
...
public abstract class CompositeCommand extends Command implements PluginIdentifiableCommand, BentoBoxCommand {
	...
	/**
	 * Controls command execution restrictions: - onlyPlayer: Command can only be
	 * used by players - onlyConsole: Command can only be used by console -
	 * configurableRankCommand: Command's required rank can be configured - hidden:
	 * Command is hidden from help and tab completion
	 */
	...
	/**
	 * Command metadata: - parameters: Localization key for command parameters -
	...
	/**
	 * Command hierarchy management: - parent: Parent command (null for top-level
	 * commands) - subCommandLevel: Depth in command tree (0 for top-level) -
	 * subCommands: Map of direct sub-commands by label - subCommandAliases: Map of
	 * sub-command aliases - usage: Full command usage string including parent chain
	...
	/**
	 * Command context: - permissionPrefix: Prefix for permission nodes (from addon)
	 * - world: World this command operates in (null for global) - addon: The addon
	 * that owns this command
	 */
	@Nullable private final String permissionPrefix;
	private World world;
	private final Addon addon;

	/**
	 * Cooldown system: Outer map: User ID → Inner map Inner map: Target ID →
	 * Expiration timestamp Target ID can be null for non-targeted cooldowns
	 */
	 ...
	 * 
	 * @param parent
	 *            - the parent composite command
	 * @param label
	 *            - string label for this subcommand
	 * @param aliases
	 ...
	 * 
	 * @param user
	 *            User who permission must be checked.
	 ...
	 * 
	 * @param ignoreHelp
	 *            Whether the Help subcommand should not be returned in the map or
	 *            not.
	 * @return Map of sub commands for this command
	 ...
	 * 
	 * @param ignoreHelp
	 *            Whether the Help subcommand should not be taken into account or
	 *            not.
	 * @return true if this command has subcommands
	...
	/**
	 * Convenience method to check if a user has a team. Consider checking the
	 * island itself {@link Island#inTeam(UUID)}
	 * 
	 * @param world
	 *            - the world to check
	 * @param user
	 ...
	 * 
	 * @param onlyPlayer
	 *            {@code true} if this command should only be run by players.
	 */
	 ...
	 * 
	 * @param onlyConsole
	 *            {@code true} if this command should only be run in the console.
	 * @since 1.24.0
	 */
	 ...
	 *
	 * @param description
	 *            The locale command's description reference to set.
	 ...
	 * different value than the default one.
	 *
	 * @param parametersHelp
	 *            The locale command's paramaters reference to set.
	 */
	...
	@Override
	@NonNull public List<String> tabComplete(final @NonNull CommandSender sender, final @NonNull String alias,
			final String[] args) {
	 ...
	 *            the CommandSender
	 * @param command
	 *            the CompositeCommand to get the subcommands from
	 * @return a list of subcommands labels or an empty list.
	 */
	@NonNull private List<String> getSubCommandLabels(@NonNull CommandSender sender, @NonNull CompositeCommand command) {
	 ...
	 */
	@Nullable public String getPermissionPrefix() {
	 ...
	 * 
	 * @param uniqueId
	 *            - the unique ID that is having the cooldown
	 * @param targetUUID
	 *            - the target (if any)
	 * @param timeInSeconds
	 *            - time in seconds to cool down
	 * @since 1.5.0
	 */
	 ...
	 * 
	 * @param uniqueId
	 *            - the UUID that is having the cooldown
	 * @param targetUUID
	 *            - the target UUID (if any)
	 * @param timeInSeconds
	 *            - time in seconds to cool down
	 */
	 ...
	 *            - the caller of the command
	 * @param uniqueId
	 *            - the id that needs to be checked
	 * @param targetUUID
	 *            - the target (if any)
	 * @return true if cool down in place, false if not
	 * @since 1.5.0
	 ...
	 * 
	 * @param hidden
	 *            whether command is hidden or not
	 * @since 1.13.0
	 */
