...
public class AdminBlueprintCommand extends ConfirmableCommand {
	...
	private Map<User, io.papermc.paper.threadedregions.scheduler.ScheduledTask> displayClipboards;
	 ...
	 * 
	 * @param user
	 *            User who need to see clipboard.
	 */
	protected void showClipboard(User user) {
		this.displayClipboards.computeIfAbsent(user, key -> {
			if (key.isPlayer()) {
				return key.getPlayer().getScheduler().runAtFixedRate(this.getPlugin(), t -> {
				...
				return Bukkit.getGlobalRegionScheduler().runAtFixedRate(this.getPlugin(), t -> {
					if (!key.isPlayer() || !key.getPlayer().isOnline()) {
						this.hideClipboard(key);
					}
