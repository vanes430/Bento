...
public class ConfirmPrompt extends StringPrompt {
	...
	public Prompt acceptInput(@NonNull ConversationContext context, String input) {
		if (input != null && input.equals(response)) {
			Bukkit.getGlobalRegionScheduler().run(plugin, t -> user.sendMessage("general.success"));
			Bukkit.getGlobalRegionScheduler().run(plugin, t -> action.run());
		} else {
			Bukkit.getGlobalRegionScheduler().run(plugin, t -> user.sendMessage("general.errors.command-cancelled"));
		}
