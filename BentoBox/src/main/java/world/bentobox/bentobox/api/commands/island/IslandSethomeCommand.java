...
public class IslandSethomeCommand extends ConfirmableCommand {
	...
	/**
	 * Cached island instance to avoid multiple database lookups. Set during
	 * canExecute and used in execute.
	 */
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		if (getIWM().isNether(user.getWorld())) {
			...
			if (ws.isRequireConfirmationToSetHomeInNether()) {
				if (!checkForceFlag(user, args, user.getTranslation("commands.island.sethome.nether.confirmation"))) {
					return false;
				}
			...
			if (ws.isRequireConfirmationToSetHomeInTheEnd()) {
				if (!checkForceFlag(user, args, user.getTranslation("commands.island.sethome.the-end.confirmation"))) {
					return false;
				}
				doSetHome(user, number);
			} else {
	...
	/**
	 * Sets the home location and notifies the user. If multiple homes exist,
	 * displays a list of all homes.
	 * 
	 * @param user
	 *            The user setting the home
	 * @param name
