...
public abstract class FlagListener implements Listener {
	 ...
	 */
	protected enum Why {
		UNPROTECTED_WORLD, OP, BYPASS_EVERYWHERE, BYPASS_ISLAND, RANK_ALLOWED, ALLOWED_IN_WORLD, ALLOWED_ON_ISLAND, NOT_ALLOWED_ON_ISLAND, NOT_ALLOWED_IN_WORLD, ERROR_NO_ASSOCIATED_USER, NOT_SET, SETTING_ALLOWED_ON_ISLAND, SETTING_NOT_ALLOWED_ON_ISLAND, SETTING_ALLOWED_IN_WORLD, SETTING_NOT_ALLOWED_IN_WORLD, NULL_LOCATION, ISLAND_DELETED
	}

	private static final String WORLD_PROTECTED = "protection.world-protected";

	@NonNull private BentoBox plugin = BentoBox.getInstance();
	@Nullable private User user = null;

	/**
	 * @return the plugin
	 */
	@NonNull public BentoBox getPlugin() {
		return plugin;
	}
	 ...
	 *            - event
	 * @param flag
	 *            - the flag that has been checked
	 */
	 ...
	 *            - event
	 * @param player
	 *            - player affected by this flag, or null if none
	 * @param loc
	 *            - location, will return true if null
	 * @param flag
	 *            - flag {@link world.bentobox.bentobox.lists.Flags}
	 ...
	 *            - flag {@link world.bentobox.bentobox.lists.Flags}
	 * @param silent
	 *            - if true, no attempt is made to tell the user
	 ...
	 * @since 1.1
	 */
	@NonNull protected Optional<Flag> getFlag(@NonNull String id) {
	 ...
	 * 
	 * @param location
	 *            location where action is taking
