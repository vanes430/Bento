...
/**
 * Provides the default implementation for the main player command in a game
 * mode (e.g., /island, /bskyblock).
 * <p>
 * This class automatically sets up all standard island management commands and
 * handles the default action when a player types the base command with no
 * arguments.
 * <p>
 * Features:
 * <ul>
...
public abstract class DefaultPlayerCommand extends CompositeCommand {
	...
	/**
	 * Creates the default player command for a game mode. Uses the first alias from
	 * config as the main label.
	 *
	 * @param addon
	 *            The game mode addon creating this command
	 */
	 ...
	 * Behavior:
	 * <ul>
	 * <li>If user has an island: Executes the configured default player action
	 * (default: "go")</li>
	 * <li>If user has no island: Executes the configured default new player action
	 * (default: "create")</li>
	 * </ul>
	 ...
	 * @param args
	 *            Command arguments (must be empty for default behavior)
	 ...
	 * <p>
	 * If the command is a registered sub-command, it will be executed through the
	 * command system. Otherwise, it will be executed directly through the server's
	 * command system, allowing integration with other plugins (e.g., DeluxeMenus).
	 *
	 * @param user
	 *            The user executing the command
	 * @param label
	 *            The command label used
	 * @param command
	 *            The configured action to execute
	 * @param defaultSubCommand
	 *            Fallback sub-command if no action is configured
	 * @return true if the command was executed successfully
	 */
