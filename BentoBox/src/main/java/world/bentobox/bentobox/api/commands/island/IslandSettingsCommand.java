 ...
 * <p>
 * This command opens a tabbed GUI panel that allows players to configure their
 * island's protection and general settings flags.
 * <p>
 * Features:
 * <ul>
 ...
 * </ul>
 * <p>
 * Permission: {@code island.settings} Aliases: settings, flags, options
 *
 * @author tastybento
 * @since 1.0
 */
public class IslandSettingsCommand extends CompositeCommand {
	...
	/**
	 * Cached island instance for the command execution. Set during canExecute and
	 * used in execute.
	 */
	...
	public boolean execute(User user, String label, List<String> args) {
		new TabbedPanelBuilder().user(user).island(island).world(island.getWorld())
				.tab(1, new SettingsTab(getWorld(), user, Flag.Type.PROTECTION))
				.tab(2, new SettingsTab(getWorld(), user, Flag.Type.SETTING)).startingSlot(1).size(54).build()
				.openPanel();
		return true;
	}
}
