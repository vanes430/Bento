...
public class AdminPurgeRegionsCommand extends CompositeCommand implements Listener {
	...
	public AdminPurgeRegionsCommand(CompositeCommand parent) {
		...
		isNether = getPlugin().getIWM().isNetherGenerate(getWorld())
				&& getPlugin().getIWM().isNetherIslands(getWorld());
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		Bukkit.getAsyncScheduler().runNow(getPlugin(), t -> findIslands(getWorld(), days));
		return true;
	}
	...
	private void deletePlayerFromWorldFolder(String islandID) {
		...
		getPlugin().getIslands().getIslandById(islandID).ifPresent(island -> island.getMemberSet().forEach(uuid -> {
			...
			if (memberOf.isEmpty()) {
				...
				// Remove the player from the world folder playerdata because they no longer
				// have any island associated with them
				if (playerData.exists()) {
					...
					if (playerFile.exists() && !playerFile.delete()) {
						getPlugin()
								.logError("Failed to delete player data backup file: " + playerFile.getAbsolutePath());
					}
				}
			}
		}));
	...
	/**
	 * Deletes a file if it exists, logging an error if deletion fails. Does not log
	 * if the parent folder does not exist (normal for entities/poi).
	 * 
	 * @param file
	 *            the file to delete
	 * @return true if deleted or does not exist, false if exists but could not be
	 *         deleted
	 */
	...
	/**
	 * Deletes all region files in deleteableRegions that are older than
	 * {@code days}. Also deletes corresponding entities and poi files in each
	 * dimension.
	 * 
	...
	private void findIslands(World world, int days) {
		try {
			...
			if (islandGrid == null) {
				if (user.isPlayer()) {
					user.getPlayer().getScheduler().run(getPlugin(), t -> user.sendMessage(NONE_FOUND), null);
				} else {
					Bukkit.getGlobalRegionScheduler().run(getPlugin(), t -> user.sendMessage(NONE_FOUND));
				}
				return;
			}
			...
			// Remove any region whose island‐set contains at least one island that either
			// isn’t found or fails the deletion check:
			deleteableRegions.values()
					.removeIf(islandIds -> islandIds.stream().map(getPlugin().getIslands()::getIslandById) // Stream<Optional<Island>>
							.anyMatch(optIsland ->
							// If missing (empty) → treat as undeletable (true) - this is a bit conservative
							// but maybe the database is messed up
							// If present, checkIsland(...) == true means “cannot delete”
							optIsland.map(this::canDeleteIsland).orElse(true)));
			// At this point any islands that might be deleted are in the cache, and so we
			...
			deleteableRegions.entrySet().stream().filter(e -> e.getValue().isEmpty())
				...
				if (user.isPlayer()) {
					user.getPlayer().getScheduler().run(getPlugin(), t -> {
						user.sendMessage("commands.admin.purge.purgable-islands", TextVariables.NUMBER,
								String.valueOf(uniqueIslands.size()));
	...
	private void displayIsland(Island island) {
		...
		if (island.isDeletable()) {
			getPlugin().log("Deletable island at " + Util.xyz(island.getCenter().toVector()) + IN_WORLD
					+ getWorld().getName() + WILL_BE_DELETED);
			return;
		}
		if (island.getOwner() == null) {
			getPlugin().log("Unowned island at " + Util.xyz(island.getCenter().toVector()) + IN_WORLD
					+ getWorld().getName() + WILL_BE_DELETED);
			return;
		}
		getPlugin().log("Island at " + Util.xyz(island.getCenter().toVector()) + IN_WORLD + getWorld().getName()
				+ " owned by " + getPlugin().getPlayers().getName(island.getOwner()) + " who last logged in "
	...
	private void displayEmptyRegion(Pair<Integer, Integer> region) {
		getPlugin().log("Empty region at r." + region.x() + "." + region.z() + IN_WORLD + getWorld().getName()
				+ " will be deleted (no islands)");
	}

	/**
	 * Formats a millisecond timestamp into a human-readable string using the
	...
	private String formatLocalTimestamp(Long millis) {
		...
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm").withZone(ZoneId.systemDefault()); // Uses
																														// the
																														// machine's
																														// local
																														// time
	...
	/**
	 * Check if an island can be deleted or not. Purge protected, spawn, or unowned
	 * islands cannot be deleted. Islands over a certain level cannot be deleted.
	 * 
	 * @param island
	 *            island
	 ...
	 *
	 * <p>
	 * If {@code nether} is {@code true}, the matching region file in the Nether
	 * (DIM-1) must also be older than the cutoff to include the coordinate. If
	 * {@code end} is {@code true}, the matching region file in the End (DIM1) must
	 * likewise be older than the cutoff. When both {@code nether} and {@code end}
	 * are {@code true}, all three dimension files must satisfy the age requirement.
	 * </p>
	 *
	 * @param days
	 *            the minimum age in days of region files to include
	 * @return a list of {@code Pair<regionX, regionZ>} for each region meeting the
	 *         age criteria
	 */
	private List<Pair<Integer, Integer>> findOldRegions(int days) {
		...
		for (File owFile : files) {
			...
			if (getRegionTimestamp(owFile) >= cutoffMillis)
				continue;
	...
	/**
	 * Maps each old region to the set of island IDs whose island‐squares overlap
	 * it.
	 *
	 * <p>
	 * Each region covers blocks [regionX*512 .. regionX*512 + 511] × [regionZ*512
	 * .. regionZ*512 + 511].
	 * </p>
	 *
	 * <p>
	 * Each IslandData provides:
	 * <ul>
	 ...
	 * </ul>
	 * The island’s maxX = minX + 2*range, maxZ = minZ + 2*range.
	 * </p>
	 *
	 * @param oldRegions
	 *            the list of region coordinates to process
	 * @param grid
	 *            a 2D TreeMap mapping centreX → (centreZ → IslandData)
	 * @return a map from region coords to the set of overlapping island IDs
	 */
	private Map<Pair<Integer, Integer>, Set<String>> mapIslandsToRegions(List<Pair<Integer, Integer>> oldRegions,
		...
		for (Pair<Integer, Integer> region : oldRegions) {
			...
			for (Map.Entry<Integer, TreeMap<Integer, IslandData>> xEntry : grid.entrySet()) {
				for (IslandData data : xEntry.getValue().values()) {
					...
					// overlap test
					boolean overlaps = !(islandMaxX < regionMinX || islandMinX > regionMaxX || islandMaxZ < regionMinZ
	...
	/**
	 * Reads a Minecraft region file (.mca) and returns the most recent per-chunk
	 * timestamp found in its header, in milliseconds since epoch.
	 *
	 * @param regionFile
	 *            the .mca file
	 * @return the most recent timestamp (in millis) among all chunk entries, or 0
	 *         if the file is invalid or empty
	 */
	private long getRegionTimestamp(File regionFile) {
		...
		try (FileInputStream fis = new FileInputStream(regionFile)) {
			...
			getPlugin().logError(
					"Failed to read region file timestamps: " + regionFile.getAbsolutePath() + " " + e.getMessage());
			return 0L;
		}
	}
}
