...
public class IslandTeamCommand extends CompositeCommand {
	...
	/**
	 * Manages the invite system in the database. Methods handle:
	 * <ul>
	...
	/**
	 * Check if a player has been invited - validates any invite that may be in the
	 * system
	 * 
	 * @param invitee
	 *            - UUID of invitee to check
	...
	public boolean isInvited(@NonNull UUID invitee) {
		...
		if (handler.objectExists(invitee.toString())) {
			@Nullable TeamInvite invite = getInvite(invitee);
			valid = getIslands().getIslandById(invite.getIslandID()).map(island -> island.isOwned() // Still owned by
																									// someone
					&& !island.isDeleted() // Not deleted
					&& !island.isDeletable() // Not deletable
					&& island.getMemberSet().contains(invite.getInviter()) // the inviter is still a member of the
																			// island
			).orElse(false);
			if (!valid) {
				// Remove invite
				CompletableFuture.runAsync(() -> handler.deleteObject(invite));
			}
		}
		return valid;
	}
	 ...
	 * @since 1.8.0
	 */
	@Nullable public UUID getInviter(UUID invitee) {
	 ...
	 * @since 1.8.0
	 */
	@Nullable public TeamInvite getInvite(UUID invitee) {
