...
public class IslandTeamInviteGUI {
	...
	public IslandTeamInviteGUI(IslandTeamCommand itc, boolean invitedCmd, Island island) {
		...
		this.itc = itc;
		// Panels
		if (!new File(plugin.getDataFolder() + File.separator + "panels", "team_invite_panel.yml").exists()) {
	...
	private PanelItem createSearchButton(ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
		...
		if (!this.searchName.isBlank()) {
			pib.description(user.getTranslation(Objects.requireNonNullElse(template.description(),
					"commands.island.team.invite.gui.button.searching"), TextVariables.NAME, searchName));
		}
		return pib.build();
	}
	...
	private PanelItem createProspectButton(ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
		...
		// Filter players based on searchName if it's not blank, and ensure they're not
		// the user
	...
	private PanelItem getProspect(Player player, ItemTemplateRecord template) {
		...
		List<String> desc = template.actions().stream().map(
				ar -> user.getTranslation("commands.island.team.invite.gui.tips." + ar.clickType().name() + ".name")
		...
		return new PanelItemBuilder().icon(player.getName()).name(player.getDisplayName()).description(desc)
				.clickHandler((panel, user, clickType, clickSlot) -> clickHandler(user, clickType, player,
						template.actions()))
				.build();
	}
	...
	class InviteNamePrompt extends StringPrompt {
		...
		@Override
		@NonNull public String getPromptText(@NonNull ConversationContext context) {
		...
		public Prompt acceptInput(@NonNull ConversationContext context, String input) {
			...
			Bukkit.getGlobalRegionScheduler().runDelayed(plugin, t -> build(user), 20L);
			return Prompt.END_OF_CONVERSATION;
		}
