...
public class AdminBlueprintSaveCommand extends ConfirmableCommand {
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		if (newFile.exists()) {
			if (!checkForceFlag(user, args, user.getTranslation("commands.admin.blueprint.file-exists"))) {
				return false;
			}
	 ...
	 * 
	 * @param user
	 *            User that triggers blueprint save.
	 * @param parent
	 *            Parent command that contains clipboard.
	 * @param clipboard
	 *            Active clipboard.
	 * @param name
	 *            Filename for the blueprint
	 * @param displayName
