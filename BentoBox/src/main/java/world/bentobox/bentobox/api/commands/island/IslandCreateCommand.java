 ...
 * <li>{@code [gamemode].island.create.[blueprintname]} - Blueprint-specific
 * permission</li>
 * </ul>
 *
 * @author tastybento
...
public class IslandCreateCommand extends CompositeCommand {
	...
	public boolean canExecute(User user, String label, List<String> args) {
		...
		@Nullable Island island = getIslands().getPrimaryIsland(getWorld(), user.getUniqueId());
		if (island != null) {
	 ...
	 * Flow:
	 * <ul>
	 * <li>With args: Creates island with specified blueprint (if valid and
	 * permitted)</li>
	 * <li>Without args:
	 * <ul>
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		if (!args.isEmpty()) {
			...
			// Make island
			Bukkit.getAsyncScheduler().runNow(getPlugin(), t -> makeIsland(user, name));
			return true;
		} else {
			...
			// Show panel only if there are bundles available
			if (!getPlugin().getBlueprintsManager().getBlueprintBundles(getAddon()).isEmpty()) {
				// Show panel
				IslandCreationPanel.openPanel(this, user, label, false);
				return true;
			}
			Bukkit.getAsyncScheduler().runNow(getPlugin(),
					t -> makeIsland(user, BlueprintsManager.DEFAULT_BUNDLE_NAME));
			return true;
		}
	}
	...
	/**
	 * Counts how many times a user has used a specific blueprint bundle. Checks
	 * island metadata for the "bundle" key matching the given name.
	 * 
	 * @param user
	 *            The user to check
	 * @param name
	...
	/**
	 * Creates a new island for the user using the specified blueprint. Also handles
	 * reset cooldown if configured. Double-checks island status before queuing to
	 * prevent race conditions.
	 *
	 * @param user
	 *            The user getting the new island
	 * @param name
	 *            The blueprint bundle name to use
	 * @return true if island creation was successful
	 */
	private boolean makeIsland(User user, String name) {
		// DOUBLE CHECK: Verify player still doesn't have an island (handles race
		// conditions)
		// This is needed because there's a time gap between canExecute() and this
		// method
		Island island = getIslands().getPrimaryIsland(getWorld(), user.getUniqueId());
		if (island != null && !island.isReserved()) {
			// Player already has an island (e.g., joined team while waiting)
			user.sendMessage("commands.island.create.already-have-island");
			return false;
		}

		// Check team member status again
		if (getIWM().getWorldSettings(getWorld()).isDisallowTeamMemberIslands()
				&& getIslands().inTeam(getWorld(), user.getUniqueId()) && island != null
				&& !user.getUniqueId().equals(island.getOwner())) {
			user.sendMessage("commands.island.create.you-cannot-make-team");
		...
		// Check concurrent islands again
		int num = this.getIslands().getNumberOfConcurrentIslands(user.getUniqueId(), getWorld());
		int max = user.getPermissionValue(
				this.getIWM().getAddon(getWorld()).map(GameModeAddon::getPermissionPrefix).orElse("") + "island.number",
				this.getIWM().getWorldSettings(getWorld()).getConcurrentIslands());
