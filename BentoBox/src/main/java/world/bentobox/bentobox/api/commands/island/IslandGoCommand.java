 ...
 * Extends {@link DelayedTeleportCommand} to provide a configurable delay before
 * teleporting to prevent abuse.
 * <p>
 * Features:
 * <ul>
 ...
 * </ul>
 * <p>
 * Aliases: go, home, h Permission: {@code island.home}
 *
 * @author tastybento
 * @since 1.0
 */
public class IslandGoCommand extends DelayedTeleportCommand {
	 ...
	 * <p>
	 * If no arguments are provided, teleports to the default home. If arguments are
	 * provided, attempts to teleport to the named home or island. Sets the
	 * teleported island as the primary island for the user.
	 */
	@Override
	public boolean execute(User user, String label, List<String> args) {
		...
		if (!args.isEmpty()) {
			...
			if (!names.containsKey(name)) {
				...
				// We know where this location is. Now work out if it's an island name or a home
				// name
				...
				if (!info.islandName) {
					...
					this.delayCommand(user, () -> getIslands().homeTeleportAsync(getWorld(), user.getPlayer(), name) // Teleport
																														// to
																														// the
																														// named
																														// home
								...
								if (user.isPlayer()) {
									user.getPlayer().getScheduler().run(getPlugin(), t -> {
										if (r) {
											...
											getPlugin().logError(user.getName()
													+ " could not teleport to their island - async teleport issue");
										}
									}, null);
								} else {
	...
	/**
	 * Checks if any of the user's islands are reserved. If a reserved island is
	 * found, redirects the user to island creation.
	 *
	 * @param user
	 *            The user to check
	 * @param islands
	 *            List of islands to check
	...
	/**
	 * Record to store island information and whether the name refers to an island
	 * name or a home location.
	 */
	...
	/**
	 * Creates a mapping of valid teleport destination names for a user. Includes:
	 * <ul>
	 ...
	 *
	 * @param user
	 *            The user whose destinations to map
	 * @param world
	 *            The world to check
