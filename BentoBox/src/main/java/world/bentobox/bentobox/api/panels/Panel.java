...
public class Panel implements HeadRequester, InventoryHolder {
	...
	/**
	 * Various types of Panels that can be created that use InventoryTypes. <br>
	 * The current list of inventories that cannot be created are:<br>
	 * <blockquote> {@link Type#INVENTORY}, {@link Type#HOPPER},
	 * {@link Type#DROPPER}, {@link Type#ANVIL} </blockquote>
	 *
	 * These relate to the Bukkit inventories with INVENTORY being the standard
	 * CHEST inventory. See {@link org.bukkit.event.inventory.InventoryType}.
	 * 
	 * @since 1.7.0
	 */
	public enum Type {
	...
	public void open(Player... players) {
		for (Player player : players) {
			Bukkit.getGlobalRegionScheduler().run(BentoBox.getInstance(), t -> {
	 ...
	 * 
	 * @param users
	 *            - users that should see the panel
	 */
