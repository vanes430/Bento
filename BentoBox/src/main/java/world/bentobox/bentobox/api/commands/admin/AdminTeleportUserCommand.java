...
/**
 * Enables admins to teleport a player to another player's island, nether or end
 * islands,
 * <p>
 * For example /acid tp lspvicky tastybento [island name] would teleport
 * lspvicky to tastybento's [named] island
 * 
 */
public class AdminTeleportUserCommand extends CompositeCommand {
	 ...
	 *            - parent command
	 * @param tpCommand
	 *            - should be "tpuser", "tpusernether" or "tpuserend"
	 */
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		new SafeSpotTeleport.Builder(getPlugin()).entity(player).location(warpSpot).failureMessage(failureMessage)
				.thenRun(() -> {
					if (user.isPlayer()) {
						user.getPlayer().getScheduler().run(getPlugin(), t -> user.sendMessage("general.success"),
								null);
					} else {
						Bukkit.getGlobalRegionScheduler().run(getPlugin(), t -> user.sendMessage("general.success"));
					}
				}).build();
		return true;
	}
