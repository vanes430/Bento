...
public abstract class DelayedTeleportCommand extends CompositeCommand implements Listener {
	...
	/**
	 * Tracks pending delayed commands. Key: Player UUID Value: DelayedCommand
	 * containing the command to run, cancellation task, and original location
	 */
	...
	/**
	 * Handles command cancellation when a player moves. Cancels the scheduled task
	 * and notifies the player.
	 * 
	 * @param uuid
	 *            UUID of the player who moved
	 */
	...
	/**
	 * Monitors player teleports and cancels delayed commands if the player
	 * teleports. This prevents players from using other teleport methods to bypass
	 * the delay. Uses Canvas EntityPostTeleportAsyncEvent for Folia compatibility.
	 *
	 * @param e
	 *            Entity post teleport event
	 */
	@EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
	public void onPlayerTeleport(EntityPostTeleportAsyncEvent e) {
	...
	/**
	 * Initiates a delayed command execution that requires the player to stand
	 * still.
	 * <p>
	 ...
	 * @param message
	 *            Optional context message (must be pre-translated)
	 * @param confirmed
	...
	public void delayCommand(User user, String message, Runnable confirmed) {
		if (getSettings().getDelayTime() < 1 || user.isOp()
			...
			if (user.isPlayer()) {
				user.getPlayer().getScheduler().run(getPlugin(), t -> confirmed.run(), null);
			} else {
				Bukkit.getGlobalRegionScheduler().run(getPlugin(), t -> confirmed.run());
			}
			return;
		}
		...
		if (user.isPlayer()) {
			task = user.getPlayer().getScheduler().runDelayed(getPlugin(), t -> {
				if (user.isPlayer()) {
					user.getPlayer().getScheduler().run(getPlugin(), t2 -> toBeMonitored.get(uuid).runnable().run(),
							null);
				} else {
					Bukkit.getGlobalRegionScheduler().run(getPlugin(), t2 -> toBeMonitored.get(uuid).runnable().run());
				}
				toBeMonitored.remove(uuid);
			}, null, getPlugin().getSettings().getDelayTime() * 20L);
		} else {
			task = Bukkit.getGlobalRegionScheduler().runDelayed(getPlugin(), t -> {
				if (user.isPlayer()) {
					user.getPlayer().getScheduler().run(getPlugin(), t2 -> toBeMonitored.get(uuid).runnable().run(),
							null);
				} else {
	...
	/**
	 * Convenience method for {@link #delayCommand(User, String, Runnable)} with no
	 * message.
	 *
	 * @param user
	 *            The user who triggered the command
	 ...
	 *
	 * @param runnable
	 *            The action to execute when the delay completes
	 * @param task
	 *            The cancellation task that will run if the player moves
	 * @param location
	 *            The player's original location for movement comparison
	 */
	private record DelayedCommand(Runnable runnable, io.papermc.paper.threadedregions.scheduler.ScheduledTask task,
			Location location) {
	}
}
