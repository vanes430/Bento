...
public class AdminSetSpawnPointCommand extends ConfirmableCommand {
	...
	public boolean execute(User user, String label, List<String> args) {
		...
		if (optionalIsland.isPresent()
			...
			if (!checkForceFlag(user, args, user.getTranslation("commands.admin.setspawnpoint.confirmation"))) {
				return false;
			}
			setSpawnPoint(user, optionalIsland.get());
	 ...
	 * 
	 * @param user
	 *            User who initiate spawn point change.
	 * @param island
	 *            Island which is targeted by user.
	 */
