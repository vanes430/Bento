...
public class BlueprintEntity {
	...
	@Expose
	String MMStance;
	// GSON can serialize records, but the record class needs to be known in
	// advance. So this breaks out the record entries
	@Expose
	String MMtype;
	// Npc storage
	@Expose
	 ...
	 * @since 3.2.6
	 */
	public record ItemFrameRec(@Expose ItemStack item, @Expose Rotation rotation, @Expose boolean isFixed,
			@Expose boolean isVisible, @Expose float dropChance) {
	}
	/**
	 * Item in ItemFrames
	 * 
	...
	public BlueprintEntity(Entity entity) {
		...
		if (entity instanceof ItemFrame frame) {
			this.setItemFrame(new ItemFrameRec(frame.getItem(), frame.getRotation(), frame.isFixed(), frame.isVisible(),
	...
	public void configureEntity(Entity e) {
		...
		// Shift to the in-block location (remove the 0.5 that the location serializer
		// used)
	...
	public void setDisplay(Location pos) {
		...
		if (d != null && this.displayRec != null) {
			...
			// Spawn an armor stand here so that we have a way to detect if a player
	...
	public void storeDisplay(Display disp) {
		...
		switch (disp) {
			...
			case TextDisplay td -> textDisp = new TextDisplayRec(td.getText(), td.getAlignment(),
					td.getBackgroundColor(), td.getFacing(), td.getLineWidth(), td.getTextOpacity(), td.isShadowed(),
					td.isSeeThrough(), td.isDefaultBackground());
			default -> {
			}
		}
