...
public class BlueprintPaster {
	...
	/**
	 * This tracks the stages of pasting from loading the chunk, pasting blocks,
	 * attachments, entities and then finishing.
	 */
	enum PasteState {
	 ...
	 */
	@NonNull private final Blueprint blueprint;
	 ...
	 */
	@Nullable private final Island island;
	...
	public BlueprintPaster(@NonNull BentoBox plugin, @NonNull Blueprint bp, World world, @NonNull Island island) {
		...
		// REMOVED: End offset is now handled when creating the island object
		// The island center already includes the +500 offset for End dimension

		// Force Y to sea level + 5 for Overworld islands (seaHeight = 64 + 5 = 69)
		// This ensures blueprint pastes at correct height regardless of bedrock
		// position
		if (world.getEnvironment().equals(World.Environment.NORMAL)) {
			int seaHeight = plugin.getIWM().getSeaHeight(world);
			// Calculate Y offset: seaHeight + 5 - (islandCenterY - bedrockY)
			int yOffset = (seaHeight + 5) - location.getBlockY();
			this.location.setY(this.location.getY() + yOffset);
		}

		// Force Y to sea level for End islands (seaHeight = 64)
		// End islands are pasted 6 blocks lower than Overworld (69 - 6 = 63)
		if (world.getEnvironment().equals(World.Environment.THE_END)) {
			int seaHeight = plugin.getIWM().getSeaHeight(world);
			// Calculate Y offset: seaHeight - 1 - (islandCenterY - bedrockY)
			// This makes End islands 6 blocks lower than Overworld (Overworld is seaHeight
			// + 5)
			int yOffset = (seaHeight - 1) - location.getBlockY();
			this.location.setY(this.location.getY() + yOffset);
		}
	...
	/**
	 * A record of all the "bits" of the blueprint that need to be pasted Consists
	 * of blocks, attached blocks, entities, iterators for the blocks and a speed
	 */
	private record Bits(
			/*
	 ...
	 * 
	 * @param useNMS
	 *            if true, NMS pasting will be used, otherwise Bukkit API
	...
	public CompletableFuture<Boolean> paste(boolean useNMS) {
		...
		final Map<Vector, BlueprintBlock> blocks = blueprint.getBlocks() == null
				? Collections.emptyMap()
				: blueprint.getBlocks();
		final Map<Vector, BlueprintBlock> attached = blueprint.getAttached() == null
				? Collections.emptyMap()
				: blueprint.getAttached();
		final Map<Vector, List<BlueprintEntity>> entities = blueprint.getEntities() == null
				? Collections.emptyMap()
		...
		// owner.ifPresent(user -> tellOwner(user, blocks.size(), attached.size(),
		// entities.size(), plugin.getSettings().getPasteSpeed()));
		...
		pastingTask = Bukkit.getGlobalRegionScheduler().runAtFixedRate(plugin,
				t -> pasterTask(result, owner, bits, useNMS), 1L, 1L);
	...
	private void pasteEntities(Bits bits, int count, Optional<User> owner, int pasteSpeed, boolean useNMS) {
		if (bits.it3().hasNext()) {
			...
			if (!entityMap.isEmpty()) {
				currentTask = useNMS
						? paster.pasteEntities(island, world, entityMap)
			...
			// Handled in BlueprintsManager now
			// owner.ifPresent(user ->
			// user.sendMessage("commands.island.create.pasting.dimension-done", "[world]",
			// dimensionType));
		}
	...
	private void pasteBlocksNow(Iterator<Entry<Vector, BlueprintBlock>> it, int count, int pasteSpeed, boolean useNMS) {
		...
		if (!blockMap.isEmpty()) {
			currentTask = useNMS
					? paster.pasteBlocks(island, world, blockMap)
	...
	private void loadChunk() {
		...
		currentTask = Util.getChunkAtAsync(location).thenRun(() -> {
			...
			// https://youtu.be/gmOTpIVxji8?si=DC-u4qWRTN5fdWd8
