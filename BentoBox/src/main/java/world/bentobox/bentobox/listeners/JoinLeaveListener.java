...
public class JoinLeaveListener implements Listener {
	...
	public void onPlayerJoin(final PlayerJoinEvent event) {
		...
		// Set the addon in the User object - required to have the right context for
		// prefixes in translations
		...
		if (plugin.getIslands().locationIsOnIsland(event.getPlayer(), user.getLocation())
			...
			Bukkit.getRegionScheduler().run(plugin, user.getLocation(),
					t -> plugin.getIslands().clearArea(user.getLocation()));
		}
	...
	private void updateIslandMaxTeamAndHomeSize(User user) {
		plugin.getIWM().getOverWorlds().stream()
				.flatMap(w -> plugin.getIslands().getIslands(w, user.getUniqueId()).stream()) // Flatten the
																								// List<Island> into a
																								// Stream<Island>
		...
		plugin.getIWM().getOverWorlds().stream().filter(w -> plugin.getIWM().isCreateIslandOnFirstLoginEnabled(w))
				.forEach(w -> {
					...
					if (!(plugin.getIslands().hasIsland(w, user)
						...
						if (delay <= 0) {
							Bukkit.getRegionScheduler().run(plugin, user.getLocation(), t -> createIsland.run());
						} else {
							Bukkit.getRegionScheduler().runDelayed(plugin, user.getLocation(), t -> createIsland.run(),
									delay * 20L);
						}
					}
				});
	 ...
	 * 
	 * @param world
	 *            World where cleaning must occur.
	 * @param user
	 *            Targeted user.
	 */
