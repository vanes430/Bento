...
public class BlockInteractionListener extends FlagListener {
	...
	private void checkClickedBlock(Event e, Player player, Block block) {
		...
		switch (type) {
			...
			case REDSTONE_WIRE, REPEATER, COMPARATOR, DAYLIGHT_DETECTOR ->
	...
	private boolean checkSpecialCases(Event e, Player player, Block block) {
		...
		if (Util.isVersionAtLeast("1.21.10")) {
			...
			// Prevent animation of copper golems. Use break blocks for now. This could
			// potentiall have it's own flag in the future.
	 ...
	 * 
	 * @param player
	 *            Player that must be checked.
	 * @param material
	 *            item that mus t be checked.
