...
public class PVPListener extends FlagListener {
	...
	private final Map<Integer, UUID> thrownPotions = new ConcurrentHashMap<>();
	private final Map<Entity, Player> firedFireworks = Collections.synchronizedMap(new WeakHashMap<>());
	...
	public void onEntityDamage(EntityDamageByEntityEvent e) {
		if (e.getEntity() instanceof Player player && getPlugin().getIWM().inWorld(e.getEntity().getWorld())) {
			// Allow self damage or NPC attack or attack by NPC because Citizens handles its
			// own PVP
	 ...
	 * 
	 * @param user
	 *            - user who is initiating the action
	 * @param le
	 *            - Living entity involved
	 * @param e
	 *            - event driving
	 * @param flag
	 *            - flag to check
	...
	public void onLingeringPotionSplash(final LingeringPotionSplashEvent e) {
		...
		if (e.getEntity().getShooter() instanceof Player player
			...
			Bukkit.getRegionScheduler().runDelayed(getPlugin(), e.getAreaEffectCloud().getLocation(),
					t -> thrownPotions.remove(e.getAreaEffectCloud().getEntityId()),
	...
	/**
	 * Warn visitors if the island they are teleporting to has PVP on Uses Canvas
	...
	public void onPlayerTeleport(EntityTeleportAsyncEvent e) {
		...
		getIslands().getIslandAt(e.getTo()).ifPresent(island -> {
			...
			if (island.isAllowed(Flags.PVP_OVERWORLD)) {
				alertUser(player, Flags.PVP_OVERWORLD);
			}
			if (island.isAllowed(Flags.PVP_NETHER)) {
				alertUser(player, Flags.PVP_NETHER);
			}
			if (island.isAllowed(Flags.PVP_END)) {
				alertUser(player, Flags.PVP_END);
			}
		});
	}
