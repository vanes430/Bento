...
public class CommandRankClickListener implements ClickHandler {
	...
	public boolean onClick(Panel panel, User user, ClickType clickType, int slot) {
		...
		if (!island.isAllowed(user, Flags.CHANGE_SETTINGS)) {
			String rank = user
					.getTranslation(RanksManager.getInstance().getRank(Objects.requireNonNull(island).getRank(user)));
	...
	private List<String> getCommands(World world, User user) {
		...
		return plugin.getCommandsManager().getCommands().values().stream()
				...
				.filter(c -> c.testPermission(user.getSender())) // Only allow them to see commands they have permission
																	// to see
				.flatMap(c -> getCmdRecursively("/", c).stream())
				.filter(label -> user.isOp() || !hiddenItems.contains(CommandCycleClick.COMMAND_RANK_PREFIX + label)) // Hide
																														// any
																														// hidden
																														// commands
	 ...
	 * 
	 * @param labels
	 *            - preceding command's label list
	 * @param cc
	 *            - composite command
