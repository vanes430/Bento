...
public class WebManager {
	...
	@NonNull private final BentoBox plugin;
	@Nullable private GitHubWebAPI gitHub;
	@NonNull private final List<CatalogEntry> addonsCatalog;
	@NonNull private final List<CatalogEntry> gamemodesCatalog;
	@NonNull private final Map<String, List<Contributor>> contributors;
	...
	public WebManager(@NonNull BentoBox plugin) {
		...
		if (plugin.getSettings().isGithubDownloadData()) {
			...
			if (connectionInterval <= 0) {
				...
				Bukkit.getAsyncScheduler().runDelayed(plugin, t -> requestGitHubData(), 20L * 50L,
						TimeUnit.MILLISECONDS);
			} else {
				// Set connection interval to be at least 60 minutes.
				connectionInterval = Math.max(connectionInterval, 60 * 20 * 60L);
				Bukkit.getAsyncScheduler().runAtFixedRate(plugin, t -> requestGitHubData(), 20L * 50L,
						connectionInterval * 50L, TimeUnit.MILLISECONDS);
			}
		}
	}
	...
	@NonNull private String getContent(@NonNull GitHubRepository repo, String fileName) {
		try {
	...
	private void gatherContributors(@NonNull GitHubRepository repo) {
		try {
			...
			for (GitHubContributor gitHubContributor : repo.getContributors()) {
				addonContributors
						.add(new Contributor(gitHubContributor.username(), gitHubContributor.contributionsAmount()));
			}
	 ...
	 * @since 1.5.0
	 */
	@NonNull public List<CatalogEntry> getAddonsCatalog() {
	 ...
	 * @since 1.5.0
	 */
	@NonNull public List<CatalogEntry> getGamemodesCatalog() {
	 ...
	 * @since 1.9.0
	 */
	@NonNull public List<Contributor> getContributors(String repository) {
	 ...
	 * @since 1.5.0
	 */
	@NonNull public Optional<GitHubWebAPI> getGitHub() {
