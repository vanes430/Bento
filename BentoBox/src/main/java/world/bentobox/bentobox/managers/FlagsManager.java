...
public class FlagsManager {
	...
	private final Map<@NonNull Flag, @NonNull Optional<Addon>> flags = new ConcurrentHashMap<>();
	 ...
	 */
	private final Map<@NonNull Listener, @NonNull Boolean> registeredListeners = new ConcurrentHashMap<>();
	 ...
	 * 
	 * @param addon
	 *            - addon that is registering this flag
	 * @param flag
	 *            flag to be registered
	 * @return true if successfully registered, false if not, e.g., because one with
	 *         the same ID already exists
	...
	public boolean registerFlag(@Nullable Addon addon, @NonNull Flag flag) {
		...
		flags.put(flag, Optional.ofNullable(addon));
	 ...
	 */
	@NonNull public List<Flag> getFlags() {
	 ...
	 * @since 1.1
	 */
	@NonNull public Optional<Flag> getFlag(@NonNull String id) {
	...
	public void unregister(@NonNull Addon addon) {
		...
		flags.entrySet().stream().filter(e -> e.getValue().filter(addon::equals).isPresent()).map(Map.Entry::getKey)
				.forEach(f -> f.getListener().ifPresent(HandlerList::unregisterAll));
		// Remove flags
		flags.values().removeIf(opt -> opt.filter(addon::equals).isPresent());
	}
