...
public class PlaceholdersManager {
	 ...
	 * 
	 * @param placeholder
	 *            the placeholder to register, not null. It will be appended with
	 *            {@code "bentobox_"} by the placeholder plugin.
	 * @param replacer
	 *            the expression that will return a {@code String} when executed,
	 *            which will be this placeholder's replacement.
	 */
	 ...
	 * 
	 * @param addon
	 *            the addon to register this placeholder on its behalf. If null, the
	 *            placeholder will be registered using
	 *            {@link #registerPlaceholder(String, PlaceholderReplacer)}.
	 * @param placeholder
	 *            the placeholder to register, not null. It will be appended with
	 *            the addon's name by the placeholder plugin.
	 * @param replacer
	 *            the expression that will return a {@code String} when executed,
	 ...
	 * 
	 * @param addon
	 *            the gamemode addon to register the default placeholders too.
	 * @since 1.5.0
	 */
	...
	private void registerOwnedIslandPlaceholders(@NonNull GameModeAddon addon) {
		...
		IntStream.range(0, maxIslands)
				.forEach(i -> registerPlaceholder(addon, "island_memberlist_" + (i + 1),
						user -> user == null
								? ""
								: plugin.getIslands().getIslands(addon.getOverWorld(), user).stream().skip(i)
										.findFirst().map(island -> island.getMemberSet().stream()
			...
			registerPlaceholder(addon, "island_member_rank_" + i, user -> {
				if (user != null) {
					...
					for (UUID uuid : island.getMemberSet(RanksManager.MEMBER_RANK)) {
						if (j++ == count) {
							return user
									.getTranslationOrNothing(RanksManager.getInstance().getRank(island.getRank(uuid)));
						}
					}
				}
				return "";
								...
								for (UUID uuid : island.getMemberSet(RanksManager.MEMBER_RANK)) {
									if (j++ == count) {
										return user.getTranslationOrNothing(
												RanksManager.getInstance().getRank(island.getRank(uuid)));
									}
								}
								return "";
							}).orElse("");
	 ...
	 * 
	 * @param placeholder
	 *            the placeholder to unregister, not null.
	 * @since 1.4.0
	 */
	 ...
	 * 
	 * @param addon
	 *            the addon that originally registered this placeholder. If null,
	 *            this placeholder will be unregistered using
	 ...
	 * @since 1.4.0
	 */
	@NonNull private Optional<PlaceholderAPIHook> getPlaceholderAPIHook() {
	 ...
	 * 
	 * @param player
	 *            the Player to get the placeholders for or null for
