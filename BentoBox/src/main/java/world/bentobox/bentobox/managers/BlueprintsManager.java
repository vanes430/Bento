...
public class BlueprintsManager {
	...
	@NonNull public static final String FOLDER_NAME = "blueprints";
	 ...
	 */
	@NonNull private final Map<GameModeAddon, List<BlueprintBundle>> blueprintBundles;
	 ...
	 */
	@NonNull private final Map<GameModeAddon, List<Blueprint>> blueprints;
	...
	@NonNull private final Set<GameModeAddon> blueprintsLoaded;
	 ...
	 *
	 * @param addon
	 *            the {@link GameModeAddon} to extract the blueprints from.
	 */
	 ...
	 *
	 * @param addon
	 *            the {@link GameModeAddon} to get the blueprint bundles.
	 */
	@NonNull public Map<String, BlueprintBundle> getBlueprintBundles(@NonNull GameModeAddon addon) {
	 ...
	 * @since 1.8.0
	 */
	@Nullable public BlueprintBundle getDefaultBlueprintBundle(@NonNull GameModeAddon addon) {
		if (blueprintBundles.containsKey(addon)) {
	 ...
	 *
	 * @param addon
	 *            the {@link GameModeAddon}
	 * @return a {@link File} instance of the blueprints folder of this
	 *         GameModeAddon.
	 */
	@NonNull private File getBlueprintsFolder(@NonNull GameModeAddon addon) {
	 ...
	 *
	 * @param addon
	 *            the {@link GameModeAddon} to load the blueprints of.
	 */
	 ...
	 *            - the island
	 * @param name
	 *            - name of bundle to paste
	 * @param task
	 *            - task to run after pasting is completed
	 * @param useNMS
	 *            - true to use NMS pasting
	...
	public boolean paste(GameModeAddon addon, Island island, String name, Runnable task, boolean useNMS) {
		...
		// Paste sequentially
		if (bp != null) {
			final Optional<User> owner = Optional.ofNullable(island.getOwner()).map(User::getInstance);
			final long[] timer = {System.currentTimeMillis()};

			owner.ifPresent(u -> u.sendMessage("commands.island.create.pasting.generating", "[world]",
					u.getTranslation("general.worlds.overworld")));
			new BlueprintPaster(plugin, bp, addon.getOverWorld(), island).paste(useNMS).thenCompose(v -> {
				long duration = System.currentTimeMillis() - timer[0];
				owner.ifPresent(u -> {
					u.sendMessage("commands.island.create.pasting.dimension-done", "[world]",
							u.getTranslation("general.worlds.overworld"), "[time]", String.valueOf(duration));
					try {
						u.getPlayer().playSound(u.getLocation(), org.bukkit.Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1f, 1f);
					} catch (Exception e) {
						/* Ignore if sound fails or player offline */ }
				});

				timer[0] = System.currentTimeMillis();
				return pasteNether(addon, bb, island, owner);
			}).thenCompose(v -> {
				if (v) {
					long duration = System.currentTimeMillis() - timer[0];
					owner.ifPresent(u -> {
						u.sendMessage("commands.island.create.pasting.dimension-done", "[world]",
								u.getTranslation("general.worlds.nether"), "[time]", String.valueOf(duration));
						try {
							u.getPlayer().playSound(u.getLocation(), org.bukkit.Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1f,
									1f);
						} catch (Exception e) {
						}
					});
				...
				timer[0] = System.currentTimeMillis();
				return pasteEnd(addon, bb, island, owner);
			}).thenAccept(v -> {
				if (v) {
					long duration = System.currentTimeMillis() - timer[0];
					owner.ifPresent(u -> {
						u.sendMessage("commands.island.create.pasting.dimension-done", "[world]",
								u.getTranslation("general.worlds.the-end"), "[time]", String.valueOf(duration));
						try {
							u.getPlayer().playSound(u.getLocation(), org.bukkit.Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1f,
									1f);
						} catch (Exception e) {
				...
				sendMessage(island);
				Bukkit.getRegionScheduler().run(plugin, island.getCenter(), t -> task.run());
			});
		}
		// Set the bundle name
	...
	private CompletableFuture<Boolean> pasteNether(GameModeAddon addon, BlueprintBundle bb, Island island,
		...
		if (bb.getBlueprint(World.Environment.NETHER) != null && addon.getWorldSettings().isNetherGenerate()
			...
			// Only send message if we are actually proceeding
			owner.ifPresent(u -> u.sendMessage("commands.island.create.pasting.generating", "[world]",
					u.getTranslation("general.worlds.nether")));
	...
	private CompletableFuture<Boolean> pasteEnd(GameModeAddon addon, BlueprintBundle bb, Island island,
		...
		if (bb.getBlueprint(World.Environment.THE_END) != null && addon.getWorldSettings().isEndGenerate()
			...
			// Only send message if we are actually proceeding
			owner.ifPresent(u -> u.sendMessage("commands.island.create.pasting.generating", "[world]",
					u.getTranslation("general.worlds.the-end")));
	 ...
	 * 
	 * @param island
	 *            Island which owner must receive a message.
	 */
	 ...
	 *
	 * @param addon
	 *            - addon making the request
	 * @param user
	 *            - user making the request
	 * @param name
	 *            - name of the blueprint bundle
