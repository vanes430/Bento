...
public class IslandGrid {
	...
	public boolean addToGrid(Island island) {
		...
		/*
		 * Find any existing islands that could overlap: - Any existing island with minX
		 * <= newMaxX could extend over newMinX, so we must consider all entries with
		 * key <= newMaxX (use headMap). - For each candidate X entry, consider Z
		 * entries with minZ <= newMaxZ (use headMap). This avoids missing large islands
		 * whose minX is far left of the new island.
		 */
	...
	/**
	 * Retrieves the island located at the specified x and z coordinates, covering
	 * both the protected area and the full island space. Returns null if no island
	 * exists at the given location. This will load the island from the database if
	...
	public @Nullable String getIslandStringAt(int x, int z) {
		...
		// Attempt to find the closest z-coordinate entry that does not exceed 'z'
		// within the found x-coordinate
