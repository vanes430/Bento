...
public class HeadGetter {
	 ...
	 * 
	 * @param cache
	 *            Cache object that need to be added into local cache.
	 * @since 1.14.1
	 */
	...
	private void runPlayerHeadGetter() {
		Bukkit.getAsyncScheduler().runAtFixedRate(plugin, t -> {
			synchronized (HeadGetter.names) {
				...
				while (!HeadGetter.names.isEmpty() && counter < plugin.getSettings().getHeadsPerCall()) {
					...
					Bukkit.getAsyncScheduler().runNow(plugin, t2 -> {
							...
							for (HeadRequester req : HeadGetter.headRequesters.get(userName)) {
								...
								if (!plugin.isShutdown()) {
									...
									Bukkit.getAsyncScheduler().runNow(this.plugin,
											t3 -> req.setHead(elementEntry.getValue()));
								}
							}
						}
					});
		...
		}, 1L * 50L, plugin.getSettings().getTicksBetweenCalls() * 50L, TimeUnit.MILLISECONDS);
	}
	 ...
	 * 
	 * @param name
	 *            user which Id must be returned.
	 ...
	 *
	 * @param requestedUrl
	 *            Url which content must be returned.
