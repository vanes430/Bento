...
public class Util {
	 ...
	 *
	 * @param location
	 *            - the location location to query
	 ...
	 *
	 * @param s
	 *            - serialized location in format "world:x:y:z:y:p"
	 * @return Location
	 */
	 ...
	 * 
	 * @param user
	 *            - the User - if null, all player names on the server are shown
	 ...
	 */
	@Nullable public static World getWorld(@Nullable World world) {
		if (world == null) {
			return null;
		}
	 ...
	 * @since 1.3.0
	 */
	@Nullable public static Date parseGitHubDate(@NonNull String gitHubDate) {
	...
	public static boolean isPassiveEntity(Entity entity) {
		...
		boolean isPassiveByClass = entity instanceof Animals || entity instanceof IronGolem || entity instanceof Snowman
				|| entity instanceof Bat || entity instanceof Allay;
		...
		// Check for newer entity types by their enum name (String comparison is safe
		// across versions)
	 ...
	 */
	@NonNull public static CompletableFuture<Boolean> teleportAsync(@Nonnull Entity entity, @Nonnull Location location) {
		return entity.teleportAsync(location, TeleportCause.UNKNOWN);
	}
	 ...
	 *            The Location to Teleport to
	 * @param cause
	 *            The cause for the teleportation
	 * @return Future that completes with the result of the teleport
	 */
	@NonNull public static CompletableFuture<Boolean> teleportAsync(@Nonnull Entity entity, @Nonnull Location location,
			TeleportCause cause) {
	 ...
	 * 
	 * @param loc
	 *            Location to get chunk for
	 * @return Future that completes with the chunk
	 */
	@NonNull public static CompletableFuture<Chunk> getChunkAtAsync(@NonNull Location loc) {
		return Objects.requireNonNull(loc.getWorld()).getChunkAtAsync(loc.getBlockX() >> 4, loc.getBlockZ() >> 4, true);
	}
	 ...
	 *            Location to get chunk for
	 * @param gen
	 *            Should the chunk generate or not. Only respected on some MC
	 ...
	 */
	@NonNull public static CompletableFuture<Chunk> getChunkAtAsync(@NonNull Location loc, boolean gen) {
		return Objects.requireNonNull(loc.getWorld()).getChunkAtAsync(loc.getBlockX() >> 4, loc.getBlockZ() >> 4, gen);
	}
	 ...
	 * 
	 * @param world
	 *            World to load chunk for
	 * @param x
	 *            X coordinate of the chunk to load
	 * @param z
	 *            Z coordinate of the chunk to load
	 * @return Future that completes with the chunk
	 */
	@NonNull public static CompletableFuture<Chunk> getChunkAtAsync(@Nonnull World world, int x, int z) {
	 ...
	 *         exists and generation was not requested.
	 */
	@NonNull public static CompletableFuture<Chunk> getChunkAtAsync(@Nonnull World world, int x, int z, boolean gen) {
	 ...
	 * 
	 * @param loc
	 *            Location to check if the chunk is generated
	 ...
	 *            World to check for
	 * @param x
	 *            X coordinate of the chunk to check
	 * @param z
	 *            Z coordinate of the chunk to checl
	 * @return If the chunk is generated or not
	 */
	public static boolean isChunkGenerated(@Nonnull World world, int x, int z) {
		return isChunkGenerated(world, x, z, false);
	}
	 ...
	 *            X coordinate of the chunk to check
	 * @param z
	 *            Z coordinate of the chunk to check
	 * @param blocking
	 *            If true, the check will block if the chunk is not loaded.
	 * @return If the chunk is generated or not. Returns true if blocking is false
	 *         and chunk is not loaded.
	 */
	public static boolean isChunkGenerated(@Nonnull World world, int x, int z, boolean blocking) {
		if (!blocking) {
			// To avoid watchdog timeouts during spiral search, we return false
			// (ungenerated).
			// This is safe because BentoBox already checks its internal database for
			// existing islands.
			return false;
		}
	...
	public static boolean isChunkLoaded(@Nonnull World world, int x, int z) {
		// Paper has a non-blocking isChunkLoaded
	 ...
	 * <ul>
	 * <li>The major, minor, and patch components of the given version are greater
	 * than or equal to those of the required version.</li>
	 * <li>If the numeric components are equal, the absence of "-SNAPSHOT" in the
	 * given version takes precedence (i.e., release versions are considered more
	 * compatible than SNAPSHOT versions).</li>
	 * </ul>
	 * </p>
	 * 
	 * @param version
	 *            the version to check, in the format
	 *            "major.minor.patch[-SNAPSHOT]".
	 * @param requiredVersion
	 *            the required version, in the format
	 *            "major.minor.patch[-SNAPSHOT]".
	 * @return {@code true} if the given version is compatible with the required
	 *         version; {@code false} otherwise.
	 * 
	 *         <p>
	 *         Examples:
	 *         <ul>
	 *         <li>{@code isVersionCompatible("2.1.0", "2.0.0-SNAPSHOT")} returns
	 *         {@code true}</li>
	 *         <li>{@code isVersionCompatible("2.0.0", "2.0.0-SNAPSHOT")} returns
	 *         {@code true}</li>
	 *         <li>{@code isVersionCompatible("2.0.0-SNAPSHOT", "2.0.0")} returns
	 *         {@code false}</li>
	 *         <li>{@code isVersionCompatible("1.9.9", "2.0.0-SNAPSHOT")} returns
	 *         {@code false}</li>
	 *         </ul>
	 *         </p>
	 */
	public static boolean isVersionCompatible(String version, String requiredVersion) {
		...
		// If required version is a full release but current version is SNAPSHOT, it's
		// incompatible
	 ...
	 * 
	 * @param textToColor
	 *            Text which color codes must be parsed.
	 * @return String text with parsed colors and stripped whitespaces after them.
	 */
	@SuppressWarnings("deprecation")
	@NonNull public static String translateColorCodes(@NonNull String textToColor) {
	 ...
	 * @since 1.9.0
	 */
	@NonNull public static String stripSpaceAfterColorCodes(String textToStrip) {
		if (textToStrip == null)
			return "";
	 ...
	 *            the input.
	 * @param parse
	 *            whether the input should be checked to ensure it can be parsed as
	 ...
	 * @since 1.13.0
	 */
	@Nullable public static UUID getUUID(@NonNull String nameOrUUID) {
	 ...
	 * 
	 * @param user
	 *            - user affected by the commands
	 * @param commands
	 *            - a list of commands
	 * @param commandType
	 *            - the type of command being run - used in the console error
	 *            message
	 */
	 ...
	 *            - user affected by the commands
	 * @param ownerName
	 *            - name of the island owner, or the user's name if it is the user's
	 *            island
	 * @param commands
	 *            - a list of commands
	 * @param commandType
	 ...
	 *
	 * @param localeKey
	 *            locale key for the message to broadcast
	 * @param variables
	 *            any variables for the message
	 ...
	 * 
	 * @param input
	 *            Input that need to be sanitized.
	 * @return A sanitized input without illegal characters in names.
	 */
	public static String sanitizeInput(String input) {
		return Util.stripColor(Util.translateColorCodes(input.replaceAll("[\\\\/:*?\"<>|\s]", "_"))).toLowerCase();
	}

	/**
	 * Attempts to find the first matching enum constant from an array of possible
	 * string representations. This method sequentially checks each string against
	 * the enum constants of the specified enum class by normalizing the string
	 * values to uppercase before comparison, enhancing the likelihood of a match if
	 * the enum constants are defined in uppercase.
	 *
	 * @param enumClass
	 *            the Class object of the enum type to be checked against
	 * @param values
	 *            an array of string values which are potential matches for the enum
	 *            constants
	 * @param <T>
	 *            the type parameter of the enum
	 * @return the first matching enum constant if a match is found; otherwise,
	 *         returns null
	 */
	...
	/**
	 * Simple utility method to check if the server version is at least the target
	 * version.
	 */
	...
	private static final LegacyComponentSerializer LEGACY_SERIALIZER = LegacyComponentSerializer.builder()
			.character('&').hexColors() // Enables support for modern hex codes (e.g., &#FF0000) alongside legacy codes.
			.build();

	/**
	 * Converts a string containing Bukkit color codes ('&') into an Adventure
	 * Component.
	 *
	 * @param legacyString
	 *            The string with Bukkit color and format codes.
