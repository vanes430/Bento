...
public class IslandInfo {
	...
	public void showAdminInfo(User user, Addon addon) {
		...
		island.getMetaData("bundle")
				.ifPresent(mdv -> user.sendMessage("commands.admin.info.bundle", TextVariables.NAME, mdv.asString()));
	...
	public boolean showInfo(User user) {
		...
		user.sendMessage("commands.admin.info.protection-range", RANGE, String.valueOf(island.getProtectionRange()));
		user.sendMessage("commands.admin.info.protection-coords", XZ1,
				Util.xyz(new Vector(island.getMinProtectedX(), world.getMinHeight(), island.getMinProtectedZ())), XZ2,
				Util.xyz(new Vector(island.getMaxProtectedX() - 1, world.getMaxHeight(),
