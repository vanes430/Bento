...
public class SafeSpotTeleport {
	...
	void tryToGo(String failureMessage) {
		...
		task = Bukkit.getGlobalRegionScheduler().runAtFixedRate(plugin, t -> gatherChunks(failureMessage), 1L, SPEED);
	}
		...
		if (portal && bestSpot != null) {
			...
			entity.getScheduler().run(plugin, task -> {
				...
				if (failRunnable != null) {
					entity.getScheduler().run(plugin, t2 -> failRunnable.run(), null);
				}
				result.complete(false);
			}, null);
		} else {
			if (failRunnable != null) {
				entity.getScheduler().run(plugin, t -> failRunnable.run(), null);
			}
			result.complete(false);
		}
	}
	...
	void teleportEntity(@NonNull final Location loc) {
		...
		entity.getScheduler().run(plugin, task -> {
			...
			Util.teleportAsync(Objects.requireNonNull(entity), Objects.requireNonNull(loc)).thenRun(() -> {
				if (runnable != null)
					entity.getScheduler().run(plugin, task2 -> runnable.run(), null);
				result.complete(true);
			});
		}, null);
	}
	...
	public static class Builder {
		 ...
		 *            failure message to report to user
		 * @return Builder
		 */
		 ...
		 * @since 1.14.0
		 */
		@Nullable public CompletableFuture<Boolean> buildFuture() {
			build();
			return result;
		}
		 ...
		 */
		@Nullable public SafeSpotTeleport build() {
			// Error checking
			if (entity == null) {
