...
public class ClosestSafeSpotTeleport {
	...
	void checkLocation() {
		...
		this.task = Bukkit.getGlobalRegionScheduler().runAtFixedRate(this.plugin, t -> gatherChunks(), 1L,
				CHUNK_LOAD_SPEED);
	}
	 ...
	 * 
	 * @param chunksToScan
	 *            List of chunks that will be scanned.
	 * @param blockCoord
	 *            Block coordinates that must be in island.
	 * @param chunkCoord
	 *            Chunk coordinate.
	 */
	 ...
	 * 
	 * @param chunkSnapshot
	 *            Spigot Chunk Snapshot with blocks.
	 */
	...
	void finishTask() {
		...
		if (this.portal && this.noPortalPosition != null) {
			...
			// Schedule on the target world's region to avoid cross-region access in Folia
			Bukkit.getRegionScheduler().run(this.plugin, this.location, t -> returnAndTeleport(player));
		}
	...
	void returnAndTeleport(Player player) {
		...
		// Check highest block - now safe to call as we're in the correct region thread
	 ...
	 * 
	 * @param baseMaterial
	 *            Material that will be for top and bottom block.
	 */
	...
	void teleportEntity(final Location location) {
		// Return to main thread and teleport the player
		this.entity.getScheduler().run(this.plugin, t -> this.asyncTeleport(location), null);
	}
	 ...
	 * 
	 * @param location
	 *            Location where player should be teleported.
	 */
	void asyncTeleport(final Location location) {
		Util.teleportAsync(Objects.requireNonNull(entity), Objects.requireNonNull(location)).thenRun(() -> {
			if (this.successRunnable != null) {
				this.entity.getScheduler().run(this.plugin, t -> this.successRunnable.run(), null);
			}
	 ...
	 * 
	 * @param positionData
	 *            Position data that must be checked.
	 ...
	 * @param block
	 *            Block material on which player will be placed.
	 * @param spaceOne
	 *            Material one block above block.
	 * @param spaceTwo
	 *            Material two blocks above block.
	 * @param distance
	 *            Distance till the position.
	 */
	...
	public static class Builder {
		 ...
		 */
		@Nullable public ClosestSafeSpotTeleport build() {
			// Error checking
			if (this.entity == null) {
