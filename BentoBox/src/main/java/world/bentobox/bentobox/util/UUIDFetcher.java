...
public class UUIDFetcher {
	...
	@Nullable public static UUID getUUID(@NotNull String name) {
		name = name.toLowerCase(); // Had some issues with upper-case letters in the username, so I added this to
		...
		try {
			...
			try (BufferedReader bufferedReader = new BufferedReader(
				...
				return UUID.fromString(data.get("data").getAsJsonObject().get("player").getAsJsonObject().get("id") // Grab
																													// the
																													// UUID.
						.getAsString());
			}
