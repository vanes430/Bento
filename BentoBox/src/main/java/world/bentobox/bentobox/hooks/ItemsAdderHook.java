...
/**
 * Hook to provide ItemsAdder API without the Addon needing to call it directly.
 * Also provides Island Deletion support.
 */
public class ItemsAdderHook extends Hook {

	/**
	 * This flag allows to switch which island member group can use explosive items
	 * from Items Adder.
	 */
	public static final Flag ITEMS_ADDER_EXPLOSIONS = new Flag.Builder("ITEMS_ADDER_EXPLOSIONS", Material.TNT)
			.type(Flag.Type.PROTECTION).defaultRank(RanksManager.MEMBER_RANK)
			.clickHandler(new CycleClick("ITEMS_ADDER_EXPLOSIONS", RanksManager.VISITOR_RANK, RanksManager.OWNER_RANK))
			.build();
	...
	/**
	 * Remove the CustomBlock at location. This is not an efficient call so use
	 * chunk-related calls for large clearances
	 * 
	 * @param location
	 *            location
	 */
	...
	/**
	 * Returns a list of all the registered blocks identifiers in the format
	 * {@code namespace:id}
	 * 
	...
	/**
	 * Gets the Namespace and ID in the format {@code namespace:id} of the placed
	 * CustomBlock in a specific location.
	 *
	 * @param loc
	 *            the location to check
	 * @return the Namespace and ID in the format {@code namespace:id} or null if
	 *         it's not a CustomBlock.
	 */
	...
	/**
	 * Places a CustomBlock provided through the namespace and ID at the provided
	 * location. If the item does not exist it will fail silently.
	 *
	 * @param namespacedId
	 *            Namespace and ID in the format {@code namespace:id}
	 * @param loc
	 *            The location to place the CustomBlock
	 */
	...
	@Nullable public List<Location> getAllBlocksLocationsList(Chunk chunk) {
		return CustomBlock.Advanced.getAllBlocksLocationsList(chunk);
	}

	@Nullable public Map<Location, String> getAllBlocksLocations(Chunk chunk) {
	...
	/**
	 * Returns true if the registry contains a block with the specified namespaced
	 * id in the format {@code namespace:id}
	 * 
	 * @param namespacedId
	 *            Namespace and ID in the format {@code namespace:id}
	...
	/**
	 * Gets a CustomBlock instance through the provided namespace and ID. <br>
