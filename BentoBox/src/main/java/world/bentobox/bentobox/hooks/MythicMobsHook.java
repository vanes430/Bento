...
public class MythicMobsHook extends Hook {
	...
	public MythicMobRecord getMythicMob(Entity bukkitEntity) {
		...
		if (mm != null) {
			return new MythicMobRecord(mm.getMobType(), mm.getDisplayName(), mm.getLevel(), mm.getPower(),
					mm.getStance());
		}
		return null;
	}
	...
	public boolean spawnMythicMob(MythicMobRecord mmr, Location spawnLocation) {
		...
		return MythicBukkit.inst().getMobManager().getMythicMob(mmr.type()).map(mob -> {
			// A delay is required before spawning, I assume because the blocks are pasted
			// using NMS
			Bukkit.getRegionScheduler().runDelayed(getPlugin(), spawnLocation, t -> {
				// spawns mob
