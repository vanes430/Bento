...
/**
 * Provides copy and pasting of ZNPCS Plus in blueprints
 * <a href="https://github.com/Pyrbu/ZNPCsPlus">...</a>
 *
 * @author tastybento
 * @since 3.2.0
 */
public class ZNPCsPlusHook extends NPCHook {
	...
	public List<String> getNPCsInChunk(Chunk chunk) {
		return NpcApiProvider.get().getNpcRegistry().getAll().stream()
				...
				.filter(npc -> npc.getNpc().getLocation().toBukkitLocation(chunk.getWorld()).getChunk().equals(chunk)) // Only
																														// in
																														// this
																														// chunk
