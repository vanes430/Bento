...
public class MultipaperHook extends Hook {
	 ...
	 *
	 * @return True if the chunk is an external chunk, or false if the chunk is
	 *         running on this server or if it's unloaded.
	 */
	 ...
	 *
	 * @return True if the chunk is a local chunk, or false if the chunk is running
	 *         on an external server or if it's unloaded.
	 */
	 ...
	 */
	@NonNull public static String getLocalServerName() {
	 ...
	 */
	@Nullable public static String getExternalServerName(Player player) {
		return MultiLib.getExternalServerName(player);
	}

	/**
	 * Returns cross-server data that is stored under the specified key. Note that
	 * all plugins share the same set of keys. This data is non-persistent, it will
	 * be lost when the player disconnects.
	 *
	 * @param key
	...
	/**
	 * Store cross-server data under the specified key. Note that all plugins share
	 * the same set of keys. This data is non-persistent, it will be lost when the
	 * player disconnects.
	 *
	 * @param key
	 *            The key to store the data under.
	 * @param value
	 *            The data to store under the key.
	 */
	...
	/**
	 * Returns cross-server data that is stored under the specified key. Note that
	 * all plugins share the same set of keys. This data is persistent, it will be
	 * saved even if the player disconnects. This persistent data is saved onto the
	 * player's .dat file.
	 *
	 * @param key
	 *            The key the data is stored under.
	...
	/**
	 * Store cross-server data under the specified key. Note that all plugins share
	 * the same set of keys. This data is persistent, it will be saved even if the
	 * player disconnects. This persistent data is saved onto the player's .dat
	 * file.
	 *
	 * @param key
	 *            The key to store the data under.
	 ...
	 *
	 * @param plugin
	 *            The plugin listening to these notifications
	 * @param channel
	 *            The notification channel to listen to
	 * @param callback
	 *            A handler for any data received
	 */
	 ...
	 * @param channel
	 *            The notification channel to listen to
	 * @param callbackWithReply
	 *            A handler for any data received, and a method to reply to the
	 *            server on a specified channel
	 */
	 ...
	 *
	 * @param channel
	 *            The notification channel to notify on
	 * @param data
	 *            The data to notify other servers with
	 */
	...
	/**
	 * Notify the owning server of the specified chunk. This chunk must be loaded on
	 * this server. This will notify this server if this server is the owning
	 * server.
	 *
	 * @param chunk
	 *            The loaded chunk with an owning server
	 * @param channel
	 *            The notification channel to notify on
	 * @param data
	 *            The data to notify other servers with
	...
	/**
	 * Notify the owning server of the specified player. This will notify this
	 * server if this server is the owning server.
	 *
	 * @param player
	 *            The player with an owning server
	 * @param channel
	 *            The notification channel to notify on
	 * @param data
	 *            The data to notify other servers with
	 ...
	 * asynchronous. This storage medium is hosted on the Master instance, or a yaml
	 * file when using Bukkit.
	 *
