...
/**
 * This class generates Language Panel based on user specified file with name:
 * "language_panel.yml". If file with such name is located at gamemode panels
 * directory, then that file will be used. Otherwise, file in BentoBox/panels is
 * used.
 */
public class LanguagePanel extends AbstractPanel {
	...
	@Nullable protected PanelItem createNextButton(@NonNull ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
		...
		if (template.description() != null) {
			builder.description(this.user.getTranslation(template.description(), TextVariables.NUMBER,
	 ...
	 */
	@Override
	@Nullable protected PanelItem createPreviousButton(@NonNull ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
		if (this.pageIndex == 0) {
			// There are no next elements
			return null;
		}
	 ...
	 */
	@Nullable private PanelItem createLocaleButton(ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
		if (this.elementList.isEmpty()) {
		...
		if (template.dataMap().containsKey("lang_id")) {
			// Try to find locale with requested ID. if not found, use already collected
			// locale.
	...
	private PanelItem createLocaleButton(ItemTemplateRecord template, Locale locale) {
		...
		if (template.icon() != null) {
			...
			builder.icon(
					Objects.requireNonNullElseGet(language.getBanner(), () -> new ItemStack(Material.WHITE_BANNER, 1)));
		}

		if (template.title() != null) {
			builder.name(this.user.getTranslation(this.command.getWorld(), template.title(), TextVariables.NAME,
					StringUtils.capitalize(locale.getDisplayName(this.user.getLocale()))));
		} else {
			builder.name(this.user.getTranslation(reference + "name", TextVariables.NAME,
					StringUtils.capitalize(locale.getDisplayName(this.user.getLocale()))));
		}
		...
		if (template.description() != null) {
			descriptionText = this.user.getTranslationOrNothing(template.description(), AUTHORS, authors.toString(),
					SELECTED, selected.toString());
		} else {
			descriptionText = this.user.getTranslationOrNothing(reference + "description", AUTHORS, authors.toString(),
					SELECTED, selected.toString());
		}
		...
		List<ItemTemplateRecord.ActionRecords> actions = template.actions().stream().filter(
				action -> !this.user.getLocale().equals(locale) && (SELECT_ACTION.equalsIgnoreCase(action.actionType())
