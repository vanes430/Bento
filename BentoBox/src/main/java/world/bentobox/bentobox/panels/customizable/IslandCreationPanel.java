...
/**
 * This class generates Island Creation Panel based on user specified file with
 * name: "island_creation_panel.yml". If file with such name is located at
 * gamemode panels directory, then that file will be used. Otherwise, file in
 * BentoBox/panels is used.
 */
public class IslandCreationPanel extends AbstractPanel {
	...
	/**
	 * This constant is used for button to indicate that it is Blueprint Bundle
	 * type.
	 */
	private static final String BUNDLES = "BUNDLE";
	/**
	 * This constant stores value for ERROR message that will be displayed upon
	 * failing to run creation commands.
	 */
	...
	/**
	 * true if this panel has been called by a reset command. Changes how the count
	 * of used islands is done.
	 */
	 ...
	 *
	 * @param command
	 *            CompositeCommand object
	 * @param label
	 *            The main command label
	 * @param user
	 *            User who opens panel
	 * @param reset
	 *            true if this is a reset
	 */
	IslandCreationPanel(@NonNull CompositeCommand command, @NonNull User user, @NonNull String label, boolean reset) {
		super(command, user);
	 ...
	 */
	@Override
	@Nullable protected PanelItem createNextButton(@NonNull ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
	 ...
	 */
	@Nullable private PanelItem createBundleButton(ItemTemplateRecord template, TemplatedPanel.ItemSlot slot) {
		if (this.elementList.isEmpty()) {
		...
		if (template.dataMap().containsKey("unique_id")) {
			// Try to find bundle with requested ID. if not found, use already collected
			// bundle.
	...
	private PanelItem createBundleButton(ItemTemplateRecord template, BlueprintBundle bundle) {
		...
		if (template.title() != null) {
			builder.name(this.user.getTranslation(this.command.getWorld(), template.title(), TextVariables.NAME,
					bundle.getDisplayName()));
		} else {
			builder.name(
					this.user.getTranslation(BUNDLE_BUTTON_REF + "name", TextVariables.NAME, bundle.getDisplayName()));
		}
		...
		if (template.description() != null) {
			...
			builder.description(this.user.getTranslation(BUNDLE_BUTTON_REF + "description", TextVariables.DESCRIPTION,
					String.join("\n", bundle.getDescription())));
		}
				...
				long uses = plugin.getIslands().getIslands(world, user).stream()
						.filter(is -> is.getMetaData("bundle")
								.map(mdv -> bundle.getDisplayName().equalsIgnoreCase(mdv.asString())
										&& !(reset && is.isPrimary(user.getUniqueId()))) // If this is a reset, then
																							// ignore the use of the
																							// island being reset
								.orElse(false))
						.count();
		...
		if (usedUp) {
				...
				actions.forEach(action -> {
					if (clickType == action.clickType() || action.clickType() == ClickType.UNKNOWN) {
						if (SELECT_ACTION.equalsIgnoreCase(action.actionType())) {
							user.closeInventory();
							if (user.isPlayer()) {
								user.getPlayer().getScheduler().run(plugin, t -> this.command.execute(user,
										this.mainLabel, Collections.singletonList(bundle.getUniqueId())), null);
							} else {
								Bukkit.getGlobalRegionScheduler().run(plugin, t -> this.command.execute(user,
										this.mainLabel, Collections.singletonList(bundle.getUniqueId())));
							}
			...
			List<String> tooltips = actions.stream().filter(action -> action.tooltip() != null)
					.map(action -> this.user.getTranslation(this.command.getWorld(), action.tooltip()))
					.filter(Objects::nonNull).filter(text -> !text.isBlank())
	...
	/**
	 * This method is used to open Panel outside this class. It will be much easier
	 ...
	 *            User who opens panel
	 * @param reset
	 *            true if this is an island reset
	 */
