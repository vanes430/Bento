...
public class SettingsTab implements Tab, ClickHandler {
	 ...
	 */
	@Override
	@NonNull public List<@Nullable PanelItem> getPanelItems() {
		...
		return flags.stream().map((f -> f.toPanelItem(plugin, user, world, island,
				plugin.getIWM().getHiddenFlags(world).contains(f.getID())))).toList();
	}
	...
	public Map<Integer, PanelItem> getTabIcons() {
		...
		switch (currentMode.getOrDefault(user.getUniqueId(), Mode.BASIC)) {
			...
			case EXPERT -> icons.put(7,
					new PanelItemBuilder().icon(Material.NETHER_BRICK)
							...
							.clickHandler((panel, user1, clickType, slot) -> {
								...
								new ConversationFactory(BentoBox.getInstance()).withModality(true).withLocalEcho(false)
										...
										.withFirstPrompt(new ConfirmPrompt(user, plugin,
												PROTECTION_PANEL + "reset-to-default.instructions", confirmation,
												() -> {
