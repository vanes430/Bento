...
/**
 * Common Game Mode Placeholders All of these are prefixed with the game mode's
 * name, e.g., bskykblock_
 */
public enum GameModePlaceholder {
	...
	/**
	 * Returns a comma separated list of player names that are at least COOP on this
	 * island.
	 * 
	 * @since 2.4.2
	 */
	 ...
	 * @since 1.5.0
	 */
	ISLAND_COOPS_COUNT("island_coops_count", (addon, user,
			island) -> island == null ? "" : String.valueOf(island.getMemberSet(RanksManager.COOP_RANK, false).size())),
	/**
	...
	/**
	 * Returns the number of players that are or have ever been a MEMBER on this
	 * island.
	 * 
	 * @since 3.0.0
	 */
	...
	ISLAND_MAX_HOMES("island_max_homes",
			(addon, user, island) -> island == null
					? ""
					: String.valueOf(island.getMaxHomes() == null
	...
	/**
	 * Returns a comma separated list of player names that are at least TRUSTED on
	 * this island.
	 * 
	 * @since 2.4.2
	 */
	 ...
	 * @since 1.5.2
	 */
	VISITED_ISLAND_BANS_COUNT("visited_island_bans_count", (addon, user, island) -> getVisitedIsland(addon, user)
	 ...
	 * @since 1.5.2
	 */
	VISITED_ISLAND_CENTER("visited_island_center", (addon, user, island) -> getVisitedIsland(addon, user)
			.map(value -> Util.xyz(value.getCenter().toVector())).orElse("")),
	/**
	 ...
	 * @since 1.5.2
	 */
	VISITED_ISLAND_CENTER_X("visited_island_center_x", (addon, user, island) -> getVisitedIsland(addon, user)
	 ...
	 * @since 1.5.2
	 */
	VISITED_ISLAND_CENTER_Y("visited_island_center_y", (addon, user, island) -> getVisitedIsland(addon, user)
	 ...
	 * @since 1.5.2
	 */
	VISITED_ISLAND_CENTER_Z("visited_island_center_z", (addon, user, island) -> getVisitedIsland(addon, user)
			.map(value -> String.valueOf(value.getCenter().getBlockZ())).orElse("")),
	/**
	 * Returns a comma separated list of player names that are COOP on the island
	...
	VISITED_ISLAND_COOP_LIST("visited_island_coop_list",
			(addon, user, island) -> getVisitedIsland(addon, user)
					.map(value -> value.getMemberSet(RanksManager.COOP_RANK, false).stream()
	 ...
	 * @since 1.5.2
	 */
	VISITED_ISLAND_COOPS_COUNT("visited_island_coops_count", (addon, user, island) -> getVisitedIsland(addon, user)
	...
	VISITED_ISLAND_CREATION_DATE("visited_island_creation_date",
			(addon, user, island) -> getVisitedIsland(addon, user).map(
					value -> DateFormat.getInstance().format(Date.from(Instant.ofEpochMilli(value.getCreatedDate()))))
					.orElse("")),
	/**
	 ...
	 * @since 1.16.0
	 */
	VISITED_ISLAND_LOCATION("visited_island_location", (addon, user, island) -> getVisitedIsland(addon, user)
	 ...
	 * @since 1.16.0
	 */
	VISITED_ISLAND_LOCATION_X("visited_island_location_x", (addon, user, island) -> getVisitedIsland(addon, user)
	 ...
	 * @since 1.16.0
	 */
	VISITED_ISLAND_LOCATION_Y("visited_island_location_y", (addon, user, island) -> getVisitedIsland(addon, user)
	 ...
	 * @since 1.16.0
	 */
	VISITED_ISLAND_LOCATION_Z("visited_island_location_z", (addon, user, island) -> getVisitedIsland(addon, user)
	...
	VISITED_ISLAND_MAX_HOMES("visited_island_max_homes",
			(addon, user,
					island) -> getVisitedIsland(addon, user).map(value -> String.valueOf(island.getMaxHomes() == null
							? addon.getPlugin().getIWM().getMaxHomes(island.getWorld())
							: island.getMaxHomes())).orElse("")),
	/**
	 * Returns the amount of players that are at least MEMBER on the island the
	 * player is standing on.
	 * 
	 * @since 1.5.2
	 */
	VISITED_ISLAND_MEMBERS_COUNT("visited_island_members_count", (addon, user, island) -> getVisitedIsland(addon, user)
	...
	VISITED_ISLAND_MEMBERS_LIST("visited_island_members_list",
			(addon, user, island) -> getVisitedIsland(addon, user)
					.map(value -> value.getMemberSet(RanksManager.MEMBER_RANK).stream().map(addon.getPlayers()::getName)
							.collect(Collectors.joining(",")))
					.orElse("")),
	/**
	 ...
	 * @since 1.5.2
	 */
	VISITED_ISLAND_OWNER("visited_island_owner", (addon, user, island) -> getVisitedIsland(addon, user)
	 ...
	 * @since 2.4.0
	 */
	VISITED_ISLAND_RANK("visited_island_rank", (addon, user, island) -> getVisitedIsland(addon, user)
			.map(is -> user.getTranslation(RanksManager.getInstance().getRank(is.getRank(user)))).orElse("")),
	/**
	 * Returns a comma separated list of player names that are at TRUSTED on the
	 * island the player is standing on.
	 * 
	 * @since 2.4.2
	 */
	VISITED_ISLAND_TRUSTED_LIST("visited_island_trusted_list",
			(addon, user, island) -> getVisitedIsland(addon, user)
					.map(value -> value.getMemberSet(RanksManager.TRUSTED_RANK, false).stream()
							.map(addon.getPlayers()::getName).collect(Collectors.joining(",")))
					.orElse("")),
	...
	/**
	 * Provides a count of how many players have ever joined the island as a member
	 * including the owner
	 * 
	 * @param island
	 *            island
